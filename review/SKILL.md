...
**During structural analysis of changed code**, use these sqry MCP tools:

- `mcp__sqry__complexity_metrics` — check cyclomatic complexity of changed files — flag regressions
- `mcp__sqry__find_cycles` — check if changed symbols introduced or participate in cycles
- `mcp__sqry__dependency_impact` — analyze downstream impact of changed public APIs
- `mcp__sqry__find_unused` — catch newly-dead code after refactors or API changes
- `mcp__sqry__semantic_diff` — compare structural changes between the PR branch and base branch
- `mcp__sqry__direct_callers` — verify callers of changed functions still work with the new signature
