...
4. Update `state.md`: skill=review, phase=initializing, turn=0
...
1. FIRST write it to `~/.gstack/projects/$SLUG/findings-$BRANCH.md` (this is the permanent record)
2. THEN report it in conversation
3. THEN decide whether to auto-fix or flag for user

After completing each review phase, update `~/.gstack/projects/$SLUG/state.md`:
- Update `phase:` to the next check
- Increment `turn:` — if it's a multiple of 5, run a checkpoint (see Synthetic Memory Protocol)

If you're unsure whether you already reported a finding, read `findings-$BRANCH.md` —
...
2. Write `~/.gstack/projects/$SLUG/handoff.md` with the full review summary
3. Present the summary to the user based on the FILES, not your memory

The final summary MUST match what's in `findings-$BRANCH.md`. If your memory
