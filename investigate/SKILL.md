...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/gstack/bin/gstack-learnings-search --limit "$_LEARN_SKILL_LIMIT" --query "debug investigation root cause hypothesis bug fix" --cross-project 2>/dev/null || true
else
  ~/.claude/skills/gstack/bin/gstack-learnings-search --limit "$_LEARN_SKILL_LIMIT" --query "debug investigation root cause hypothesis bug fix" 2>/dev/null || true
fi
```
