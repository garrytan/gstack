...
import { CODEX_MODEL_CONFIG_FLAG, CODEX_REVIEW_MODEL_CONFIG_FLAG, CODEX_WEB_SEARCH_FLAG, codexPreflight, codexReasoningEffortFlag } from './constants';
...
export function outsideVoiceCommand(ctx: TemplateContext, opts: OutsideCommandOptions): string {
  ...
  const codex = opts.structuredBase
    ? `codex review --base ${sh(opts.structuredBase)} ${CODEX_REVIEW_MODEL_CONFIG_FLAG} ${codexReasoningEffortFlag(opts.reasoningEffort ?? 'high')} ${CODEX_WEB_SEARCH_FLAG} < /dev/null`
    : `codex exec "$(cat "$_OUTSIDE_INPUT")" -C "$_REPO_ROOT" -s read-only ${CODEX_MODEL_CONFIG_FLAG} ${codexReasoningEffortFlag(opts.reasoningEffort ?? 'high')} ${CODEX_WEB_SEARCH_FLAG} < /dev/null`;
