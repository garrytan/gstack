...
export function outsideVoiceGuard(ctx: TemplateContext): string {
  ...
  return `# GSTACK_ACTIVE_HOST, when supplied, must identify the actual harness, never a model overlay.
if { ${own}; }; then
