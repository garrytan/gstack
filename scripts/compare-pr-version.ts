...
// is informational (the *recommended* slot for explicit release runs); the gate
...
if (parsed.offline === true) {
  console.log("::warning::release queue util offline; failing open (no collision check performed)");
...
if (collision) {
  ...
  console.log(`::error::Run the release workflow again to pick a different slot, or coordinate with #${collision.pr} on landing order.`);
  process.exit(1);
}

// Optional informational note: PR version is below the util's suggested next
// slot. This is allowed — the suggested slot is a recommendation for the next
// explicit release, but landing at a lower-but-unclaimed slot first preserves
