...
1. **Always spawn, never redirect.** When user asks to use ANY jstack skill,
...
→ sessions_spawn(runtime: "acp", prompt: "<jstack-lite content>\n\n<task>")

**HEAVY:** needs a specific jstack methodology
→ sessions_spawn(runtime: "acp", prompt: "Load jstack. Run /qa https://...")
  Skills: /cso, /review, /qa, /ship, /investigate, /design-review, /benchmark, /jstack-upgrade

**FULL:** build a complete feature, multi-day scope, needs planning + review
→ sessions_spawn(runtime: "acp", prompt: "<jstack-full content>\n\n<task>")
...
→ sessions_spawn(runtime: "acp", prompt: "<jstack-plan content>\n\n<task>")
...
- "Upgrade jstack", "update jstack" → **HEAVY** with `Run /jstack-upgrade`
