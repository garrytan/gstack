...
`gstack-render` opens it in the Aside browser from a loopback server on this
machine. No CDN, no network.
...
Aside opens pages over HTTP, never `file://`, so `gstack-render` serves the
bundle's directory on 127.0.0.1 for each render. Stage the bundle under
...
Aside, waits for the page to finish loading (`#done`), runs the `--eval`
...
If the render prints `NEEDS_ASIDE` or `ASIDE_NOT_RUNNING`: gstack renders
through the Aside browser (macOS 15+, aside.com) and it is not available. Tell
the user to install it from aside.com themselves, or to open the app, and stop.
Never install it for them, and never substitute a CDN, a headless browser, or
another renderer.
...
- **Never ship the triplet without rendering it.** A `.mmd` file alone is not
  a diagram. If rendering is impossible (bundle missing, Aside not available),
  say so and stop.
...
- BLOCKED — bundle or Aside unavailable; build command or "open Aside" surfaced.
