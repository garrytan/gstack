...
preamble-tier: 4
version: 1.0.0
description: "Ship workflow: detect + merge base branch, run tests, review diff, commit, push, create a ready-for-review PR. (gstack)"
allowed-tools:
  - Bash
  - Read
  - Write
...
| syncing docs and creating or updating the PR/MR (Steps 16-17) | `sections/pr-body.md` |
...
```bash
~/.claude/skills/gstack/bin/gstack-evidence check --label tests --expect-cmd '<exact tests-lane command from Step 5>' --label vitest --expect-cmd '<exact vitest-lane command from Step 5>' --max-age 24
```
...
> **STOP.** Before syncing docs and creating or updating the PR/MR (Steps 16-17), Read `~/.claude/skills/gstack/ship/sections/pr-body.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
~/.claude/skills/gstack/bin/gstack-review-log '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","branch":"'"$(git rev-parse --abbrev-ref HEAD)"'"}'
```
