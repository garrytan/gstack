...
SSE endpoints (`/activity/stream`, `/inspector/events`) accept the Bearer token, the HttpOnly `gstack_sse` session cookie, or a scoped `?sse=<VIEW_ONLY>` session token minted by `POST /sse-session`. The legacy `?token=<ROOT>` query-param auth is no longer supported.
