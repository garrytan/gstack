...
```
# $plan --mode Discovery --module setup-gbrain — Coding-Agent Onboarding for gbrain
...
## User-invocable
When the user types `$plan --mode Discovery --module setup-gbrain`, run this skill. Three shortcut modes:

- `$plan --mode Discovery --module setup-gbrain` — full flow (default)
- `$plan --mode Discovery --module setup-gbrain --repo` — only flip the per-remote policy for the current repo
- `$plan --mode Discovery --module setup-gbrain --switch` — only migrate the engine (PGLite ↔ Supabase)
- `$plan --mode Discovery --module setup-gbrain --resume-provision <ref>` — re-enter a previously interrupted
  Supabase auto-provision at the polling step
- `$plan --mode Discovery --module setup-gbrain --cleanup-orphans` — list + delete in-flight Supabase projects
...
```bash
$GSTACK_BIN/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `$GSTACK_BIN/gstack-gbrain-detect`
...
```bash
$GSTACK_BIN/gstack-gbrain-install
```
...
```bash
. $GSTACK_BIN/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | $GSTACK_BIN/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$($GSTACK_BIN/gstack-gbrain-supabase-provision list-orgs --json)
```
...
`$plan --mode Discovery --module setup-gbrain`." STOP.
...
trap 'echo ""; echo "gstack-gbrain: interrupted. In-flight ref: $INFLIGHT_REF"; \
      echo "Resume: $plan --mode Discovery --module setup-gbrain --resume-provision $INFLIGHT_REF"; \
...
```bash
result=$($GSTACK_BIN/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
$GSTACK_BIN/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$($GSTACK_BIN/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  $GSTACK_BIN/gstack-gbrain-mcp-verify "$MCP_URL")
...
a clear "fix and re-run $plan --mode Discovery --module setup-gbrain" message. Do NOT continue to Step 5a
...
```bash
$GSTACK_BIN/gstack-gbrain-install || exit $?
...
installed). `$plan --mode Discovery --module sync-gbrain` will SKIP the code stage cleanly per plan D12.
...
`$plan --mode Discovery --module setup-gbrain --switch`. Your original brain is untouched."). STOP.
...
```bash
current_tier=$($GSTACK_BIN/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  $GSTACK_BIN/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
  Then import iff `read-write`.

If outside a git repo OR no origin remote: skip this step with a note.

For `$plan --mode Discovery --module setup-gbrain --repo` invocations, execute ONLY Step 6 and exit.
...
artifacts (CEO plans, designs, $debug --mode Diagnose-only --module investigate reports, retros) rather than
...
$GSTACK_BIN/gstack-artifacts-init --url-form-supported "$URL_FORM"
$GSTACK_BIN/gstack-config set artifacts_sync_mode artifacts-only
...
any gbrain client. The helper creates a `git worktree` of `"${GSTACK_HOME:-$HOME/.gstack}"/`,
...
try:
    ...
    pass
")
$GSTACK_BIN/gstack-gbrain-source-wireup --strict \
  ${GBRAIN_URL:+--database-url "$GBRAIN_URL"}
```

`--strict` exits non-zero on missing prereqs (gbrain not installed, < 0.18.0,
or no `"${GSTACK_HOME:-$HOME/.gstack}"/.git` yet) so the user sees the failure rather than silently
...
curated `"${GSTACK_HOME:-$HOME/.gstack}"/` artifacts into gbrain so the retrieval surface
...
After answer:
```bash
$GSTACK_BIN/gstack-config set transcript_ingest_mode <choice>
$GSTACK_BIN/gstack-gbrain-sync --full --no-brain-sync
```
...
```markdown
## GBrain Configuration (configured by $plan --mode Discovery --module setup-gbrain)
- Mode: remote-http
- MCP URL: {MCP_URL}
...
Run `$plan --mode Discovery --module sync-gbrain` to force-refresh, `$plan --mode Discovery --module sync-gbrain --full` for full reindex.

<!-- gstack-gbrain-search-guidance:end -->
```

If Step 9 smoke test fails, skip the guidance block write entirely. The user's
next `$plan --mode Discovery --module sync-gbrain` run will re-evaluate capability and write the block when
...
echo "Set up on $(date). Smoke test for $plan --mode Discovery --module setup-gbrain." | gbrain put "$SLUG"
...
The brain trust policy controls whether gstack auto-pushes `"${GSTACK_HOME:-$HOME/.gstack}"/`
...
> shared/team brain?
>
> Personal: gstack auto-pushes "${GSTACK_HOME:-$HOME/.gstack}"/ artifacts (CEO plans, design
...
```bash
$GSTACK_BIN/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$($GSTACK_BIN/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  $GSTACK_BIN/gstack-config set artifacts_sync_mode full
...
After Steps 1-9 complete, summarize. Re-running `$plan --mode Discovery --module setup-gbrain` on a
...
```bash
$GSTACK_BIN/gstack-gbrain-detect 2>/dev/null || true
$GSTACK_BIN/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
$GSTACK_BIN/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
[ -f "${GSTACK_HOME:-$HOME/.gstack}"/.gbrain-sync-state.json ] && cat "${GSTACK_HOME:-$HOME/.gstack}"/.gbrain-sync-state.json || echo "{}"
```
...
Re-run `$plan --mode Discovery --module setup-gbrain` any time the bearer rotates or the URL moves.
```
...
`"${GSTACK_HOME:-$HOME/.gstack}"/transcripts/run-<pid>-<ts>/` and gstack-brain-sync pushes them
...
Run `$plan --mode Discovery --module setup-gbrain` again any time gbrain feels off; it's safe and idempotent.
```
...
## `$plan --mode Discovery --module setup-gbrain --cleanup-orphans` (D20)
...
- **Concurrent-run lock.** At skill start, `mkdir "${GSTACK_HOME:-$HOME/.gstack}"/.setup-gbrain.lock.d`
  (atomic). If the mkdir fails, abort with: "Another `$plan --mode Discovery --module setup-gbrain` instance
  is running. Wait for it, or `rm -rf "${GSTACK_HOME:-$HOME/.gstack}"/.setup-gbrain.lock.d` if
...
- **CLAUDE.md is the audit trail.** Always update it in Step 8 after a
  successful setup.
<!-- GSTACK2_LEGACY_BODY_END source=setup-gbrain -->
