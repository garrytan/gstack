...
```
# $plan --mode Discovery --module plan-tune — Question Tuning + Developer Profile (v1 observational)
...
1. **Consent gate.** If `question_tuning` is `false` AND
   `"${GSTACK_HOME:-$HOME/.gstack}"/.question-tuning-prompted` is missing → run `Consent + opt-in`
   below. Honor the answer with a marker write either way; do not re-prompt.
2. **Setup gate.** If `question_tuning` is `true` AND
   `"${GSTACK_HOME:-$HOME/.gstack}"/developer-profile.json`'s `declared` object is empty AND
   `"${GSTACK_HOME:-$HOME/.gstack}"/.declared-setup-prompted` is missing → run `5-Q setup` below.
   Touch the marker after setup completes OR is declined.
3. **Dream-cycle gate (Layer 8 / cathedral T10/T11).** If
   `"${GSTACK_HOME:-$HOME/.gstack}"/projects/<stable-project-id>/distillation-proposals.json` exists AND has
...
10. **"Turn it off" / "disable"** → `$GSTACK_BIN/gstack-config set question_tuning false`
11. **"Turn it on" / "enable"** → `$GSTACK_BIN/gstack-config set question_tuning true && touch "${GSTACK_HOME:-$HOME/.gstack}"/.question-tuning-prompted`
...
`"${GSTACK_HOME:-$HOME/.gstack}"/.question-tuning-prompted` is missing. The user has never been
asked.
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$($GSTACK_BIN/gstack-config get gstack_contributor 2>/dev/null || echo "false")
...
2. AskUserQuestion (use the contributor-specific framing only if `_CONTRIB=true`,
   ...
   **General framing:**
   ...
   > Logs stay local (`"${GSTACK_HOME:-$HOME/.gstack}"/projects/<stable-project-id>/question-log.jsonl`).
   >
   ...
   **Contributor framing (only if `_CONTRIB=true`):**
   ...
   > `"${GSTACK_HOME:-$HOME/.gstack}"/projects/<stable-project-id>/question-log.jsonl` — nothing leaves your
...
3. ALWAYS touch the marker, regardless of choice:
   ```bash
   touch "${GSTACK_HOME:-$HOME/.gstack}"/.question-tuning-prompted
   ```

4. If A or B: enable:
   ```bash
   $GSTACK_BIN/gstack-config set question_tuning true
   ```

5. If C: do nothing else. Tell the user: "Question tuning stays off. Re-enable
   any time with `$plan --mode Discovery --module plan-tune enable` or `gstack-config set question_tuning true`."
...
- Standalone via Step 0's setup gate: `question_tuning` is already `true`
  (user opted in via gstack-config or earlier `$plan --mode Discovery --module plan-tune enable`) AND
  `declared` is empty AND `"${GSTACK_HOME:-$HOME/.gstack}"/.declared-setup-prompted` is missing.
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   `"${GSTACK_HOME:-$HOME/.gstack}"/developer-profile.json` under `declared.{dimension}`:

   ```bash
   # Ensure profile exists
   $GSTACK_BIN/gstack-developer-profile --read >/dev/null
...
2. Touch the marker so the Setup gate doesn't re-fire:
   ```bash
   touch "${GSTACK_HOME:-$HOME/.gstack}"/.declared-setup-prompted
   ```
   ...
   anytime with `$plan --mode Discovery --module plan-tune setup` (Step 0 power-user shortcut).

3. Tell the user: "Profile set. Question tuning is on. Use `$plan --mode Discovery --module plan-tune`
...
```bash
$GSTACK_BIN/gstack-developer-profile --profile
```
...
eval "$($GSTACK_BIN/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/${PROJECT_ID:-unknown}/question-log.jsonl"
if [ ! -f "$_LOG" ]; then
  echo "NO_LOG"
else
  bun -e "
...
The user has asked to change a preference, either via the `$plan --mode Discovery --module plan-tune` menu
...
4. Write:
   ```bash
   $GSTACK_BIN/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
6. If the user was responding to an inline `tune:` during another skill, note
   ...
   `$plan --mode Discovery --module plan-tune` invocations, `source: "plan-tune"` is correct.
...
```bash
$GSTACK_BIN/gstack-developer-profile --gap
```
...
```bash
$GSTACK_BIN/gstack-question-preference --stats
eval "$($GSTACK_BIN/gstack-slug 2>/dev/null)"
eval "$($GSTACK_BIN/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/${PROJECT_ID:-unknown}/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
$GSTACK_BIN/gstack-developer-profile --profile | bun -e "
...
"
echo '---DISTILL---'
$GSTACK_BIN/gstack-distill-free-text --status
```
...
invokes via `$plan --mode Discovery --module plan-tune distill` / `dream`.

**Flow:**

1. Show the proposals:
   ```bash
   $GSTACK_BIN/gstack-distill-apply --list
   ```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   $GSTACK_BIN/gstack-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   $GSTACK_BIN/gstack-distill-apply --proposal N
   ```
...
**When this fires.** The user invokes `$plan --mode Discovery --module plan-tune distill` / `dream` /
...
1. Run distill:
   ```bash
   $GSTACK_BIN/gstack-distill-free-text
   ```

2. If `RATE_CAPPED`: tell the user "You've hit today's 3 distills/day cap.
   Run again tomorrow, or `$plan --mode Discovery --module plan-tune stats` for run history."
...
```bash
$GSTACK_BIN/gstack-distill-free-text --background
```
...
<!-- GSTACK2_LEGACY_BODY_END source=plan-tune -->
