...
```
# $plan --mode Discovery --module sync-gbrain — Keep gbrain current and teach the agent to use it

You are running the canonical "keep this brain up to date" verb. $plan --mode Discovery --module setup-gbrain
installs gbrain once; $plan --mode Discovery --module sync-gbrain runs every time the user wants the brain
...
It does NOT touch `"${GSTACK_HOME:-$HOME/.gstack}"/` indexing (the existing `gstack-gbrain-source-wireup`
owns that — never double-store).

## User-invocable

When the user types `$plan --mode Discovery --module sync-gbrain`, run this skill. Argument modes (parsed by
the skill itself, not a dispatcher binary):

- `$plan --mode Discovery --module sync-gbrain` — incremental sync (default; mtime fast-path; ~50ms steady-state)
- `$plan --mode Discovery --module sync-gbrain --full` — full code reindex via `gbrain reindex-code` (~25-35 min on a big repo). Auto-builds the call graph (`gbrain dream`) **only when it was never built**.
- `$plan --mode Discovery --module sync-gbrain --dream` — build this source's call graph (`gbrain code-callers`/`code-callees`) via a source-scoped `gbrain dream --source <id>` cycle; ~minutes; runs lock-free after the sync stages. Always forces, even if already built. Only produces a graph on a code-aware schema pack; otherwise the run reports a WARN explaining why the graph is still empty.
- `$plan --mode Discovery --module sync-gbrain --no-dream` — skip the dream cycle that `--full` would otherwise auto-run.
- `$plan --mode Discovery --module sync-gbrain --code-only` — only run the code stage; skip memory + brain-sync
- `$plan --mode Discovery --module sync-gbrain --dry-run` — preview what would sync; no writes anywhere
- `$plan --mode Discovery --module sync-gbrain --no-memory` / `--no-brain-sync` — selectively skip stages
- `$plan --mode Discovery --module sync-gbrain --quiet` — suppress per-stage output
- `$plan --mode Discovery --module sync-gbrain --refresh-cache` — force-rebuild brain-aware planning cache (v1.48; replaces /brain-refresh-context per D1 fold). Skips code + memory stages; routes to `gstack-brain-cache refresh --project <slug>`.
- `$plan --mode Discovery --module sync-gbrain --audit` — emit summary of gstack-owned pages per project + sensitive-content audit (v1.48 / D10 lifecycle). Read-only.
...
Before doing anything, check that $plan --mode Discovery --module setup-gbrain has been run on this Mac.

```bash
$GSTACK_BIN/gstack-gbrain-detect 2>/dev/null
```
...
```bash
_HASH=$($GSTACK_BIN/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$($GSTACK_BIN/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
echo "BRAIN_TRUST_POLICY[$_HASH]: $_POLICY"
```

If `_POLICY == "unset"` AND `_HASH != "local"`, AskUserQuestion per the
Step 9.5 wording in `$plan --mode Discovery --module setup-gbrain` (personal vs shared, with persistence
...
```bash
$GSTACK_BIN/gstack-config set brain_trust_policy@$_HASH personal
```
...
`"${GSTACK_HOME:-$HOME/.gstack}"/transcripts/<run-id>/` and the artifacts pipeline pushes it to
...
> "This repo's gbrain trust policy is `deny`. Run `$plan --mode Discovery --module setup-gbrain --repo` to
...
- **`no-cli`**: STOP. "Local gbrain CLI not installed. Run `$plan --mode Discovery --module setup-gbrain`
  first."
- **`missing-config`** AND `gbrain_mcp_mode == "remote-http"`: tell the user
  ...
  symbol code search needs a local PGLite. Run `$plan --mode Discovery --module setup-gbrain` and pick
...
- **`missing-config`** AND `gbrain_mcp_mode != "remote-http"`: STOP. "Local
  gbrain CLI is installed but no engine config. Run `$plan --mode Discovery --module setup-gbrain` first."
- **`broken-config`** OR **`broken-db`**: STOP with a clear message:
  ...
  engine (status: {gbrain_local_status}). Two options:
    1. Re-run $plan --mode Discovery --module setup-gbrain — Step 1.5 offers Retry / Switch to PGLite /
  ...
  Re-run $plan --mode Discovery --module sync-gbrain after.
  ```
...
still run. State is persisted to `"${GSTACK_HOME:-$HOME/.gstack}"/.gbrain-sync-state.json` via
tmp-file + atomic rename. Concurrent runs are blocked by a lock file at
`"${GSTACK_HOME:-$HOME/.gstack}"/.sync-gbrain.lock` (5-min stale-takeover).
...
```bash
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' "${GSTACK_HOME:-$HOME/.gstack}"/.gbrain-sync-state.json 2>/dev/null \
...
>
> A) Run $plan --mode Discovery --module sync-gbrain --full now (recommended)
...
> Note: options differ in kind, not coverage — no completeness score.
>
> A) Run $plan --mode Discovery --module sync-gbrain --dream now (recommended)
> B) Skip — I'll run it later
...
Capability check (per $plan --mode Engineering --module plan-eng-review §6):
...
```markdown
## GBrain Search Guidance (configured by $plan --mode Discovery --module sync-gbrain)
...
built first — run `$plan --mode Discovery --module sync-gbrain --dream` (or `--full`) if they return
...
- `"${GSTACK_HOME:-$HOME/.gstack}"/` curated memory (registered as `gstack-brain-<user>` source via
...
file globs. Run `$plan --mode Discovery --module sync-gbrain` after meaningful code changes; for ongoing
auto-sync across all worktrees, run `gbrain autopilot --install` once per
machine — gbrain's daemon handles incremental refresh on a schedule.

Safety: don't run `$plan --mode Discovery --module sync-gbrain` while `gbrain autopilot` is active — the
...
search for `## GBrain Search Guidance (configured by $plan --mode Discovery --module sync-gbrain)` heading
...
Print a status block matching `$plan --mode Discovery --module setup-gbrain` Step 10 conventions. Each row
is `[OK]/[FIX]/[WARN]/[ERR]`. Reuse `gbrain doctor --json --fast` for
informational rows but DO NOT gate the guidance block on doctor (per
$plan --mode Engineering --module plan-eng-review §6 — doctor is too strict for unrelated reasons).
...
gbrain status: GREEN
  ...
  "${GSTACK_HOME:-$HOME/.gstack}" source. OK   <gstack-brain-{user}> (page_count=<N>) — managed by $plan --mode Discovery --module setup-gbrain
  Memory sync ..... OK   <artifacts_sync_mode>
  CLAUDE.md ....... OK   ## GBrain Search Guidance present
  Last sync ....... OK   <last_sync from state file>

Run `$plan --mode Discovery --module sync-gbrain` again any time gbrain feels off; safe and idempotent.
```
...
2. **Otherwise** fall back to the `CYCLE` value from Step 3.5, with honest wording
   ...
   - `never` → `WARN call graph not built — run $plan --mode Discovery --module sync-gbrain --dream`
   - `unknown` → `WARN could not probe call graph (doctor unavailable) — run $plan --mode Discovery --module sync-gbrain --dream if code-callers returns 0`
...
check failed; CLAUDE.md guidance block REMOVED — run $plan --mode Discovery --module setup-gbrain to repair`).
...
Mac. The orchestrator acquires a lock at `"${GSTACK_HOME:-$HOME/.gstack}"/.sync-gbrain.lock` before
...
and travels with `git push`/`git pull` — NOT through `"${GSTACK_HOME:-$HOME/.gstack}"/.brain-allowlist`
(which is for `"${GSTACK_HOME:-$HOME/.gstack}"/` brain-sync only). On a different Mac with a synced
CLAUDE.md but no local gbrain, $plan --mode Discovery --module sync-gbrain detects the mismatch via the
...
- **NEEDS_CONTEXT** — $plan --mode Discovery --module setup-gbrain has not been run, or `gbrain doctor` shows
  a state that requires user decision (e.g., engine migration).
<!-- GSTACK2_LEGACY_BODY_END source=sync-gbrain -->
