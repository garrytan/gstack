...
Check diff size: `git diff <base>...HEAD --stat | tail -1`. If the diff is >200 lines, add: "Note: This is a large diff. Consider running `$plan --mode Engineering --module plan-eng-review` or `$plan --mode Full chain --module autoplan` for architecture-level review before shipping."

If CEO Review is missing, mention as informational ("CEO Review not run — recommended for product changes") but do NOT block.

For Design Review: run `source <($GSTACK_BIN/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running $design --mode Implement --module design-review for a full visual audit post-implementation." Still never block.
...
## Lazy specialist phase: tests

When the workflow reaches this phase, read `references/sections/ship/tests.md` completely and execute it before continuing. Do not summarize or skip its questions, pressure, gates, evidence, artifacts, mutation boundary, or exit behavior.

## Lazy specialist phase: test-coverage

When the workflow reaches this phase, read `references/sections/ship/test-coverage.md` completely and execute it before continuing. Do not summarize or skip its questions, pressure, gates, evidence, artifacts, mutation boundary, or exit behavior.

## Lazy specialist phase: plan-completion

When the workflow reaches this phase, read `references/sections/ship/plan-completion.md` completely and execute it before continuing. Do not summarize or skip its questions, pressure, gates, evidence, artifacts, mutation boundary, or exit behavior.

## Lazy specialist phase: review-army

When the workflow reaches this phase, read `references/sections/ship/review-army.md` completely and execute it before continuing. Do not summarize or skip its questions, pressure, gates, evidence, artifacts, mutation boundary, or exit behavior.

## Lazy specialist phase: greptile

When the workflow reaches this phase, read `references/sections/ship/greptile.md` completely and execute it before continuing. Do not summarize or skip its questions, pressure, gates, evidence, artifacts, mutation boundary, or exit behavior.



## Lazy specialist phase: adversarial

When the workflow reaches this phase, read `references/sections/ship/adversarial.md` completely and execute it before continuing. Do not summarize or skip its questions, pressure, gates, evidence, artifacts, mutation boundary, or exit behavior.
...
5. **Record the release decision** (durable cross-session memory). The bump level is a real decision the next session should not re-derive blind:
   ```bash
   $GSTACK_BIN/gstack-decision-log '{"decision":"Ship NEW_VERSION (BUMP_LEVEL)","rationale":"WHY","scope":"repo","source":"skill","confidence":9}' 2>/dev/null || true
   ```
...
## Lazy specialist phase: changelog

When the workflow reaches this phase, read `references/sections/ship/changelog.md` completely and execute it before continuing. Do not summarize or skip its questions, pressure, gates, evidence, artifacts, mutation boundary, or exit behavior.

## Step 14: TODOS.md (auto-update)

Cross-reference the project's TODOS.md against the changes being shipped. Mark completed items automatically; prompt only if the file is missing or disorganized.

Read `references/artifacts/review/TODOS-format.md` for the canonical format reference.
...
```bash
_REDACT_PREPUSH=$($GSTACK_BIN/gstack-config get redact_prepush_hook 2>/dev/null || echo "false")
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   $GSTACK_BIN/gstack-redact install-prepush-hook
   ```
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   ...
   If A: run `$GSTACK_BIN/gstack-config set redact_prepush_hook true`
   then `$GSTACK_BIN/gstack-redact install-prepush-hook`.
   If B: run `$GSTACK_BIN/gstack-config set redact_prepush_hook false`.
...
When the workflow reaches this phase, read `references/sections/ship/pr-body.md` completely and execute it before continuing. Do not summarize or skip its questions, pressure, gates, evidence, artifacts, mutation boundary, or exit behavior.

## Step 20: Persist ship metrics

Log coverage and plan completion data so `$plan --mode Discovery --module retro` can track trends:

```bash
eval "$($GSTACK_BIN/gstack-slug 2>/dev/null)" && mkdir -p "${GSTACK_HOME:-$HOME/.gstack}"/projects/${PROJECT_ID:-unknown}
```

Append to `"${GSTACK_HOME:-$HOME/.gstack}"/projects/${PROJECT_ID:-unknown}/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> "${GSTACK_HOME:-$HOME/.gstack}"/projects/${PROJECT_ID:-unknown}/$BRANCH-reviews.jsonl
```
...
Plan-tune cathedral T15. After a successful ship, surface $plan --mode Discovery --module plan-tune once
per machine. Single line, non-blocking, marker-gated so it never re-fires.

```bash
_NUDGE_MARKER="${GSTACK_HOME:-$HOME/.gstack}/.plan-tune-nudge-shown"
_QT=$($GSTACK_BIN/gstack-config get question_tuning 2>/dev/null || echo "false")
if [ ! -f "$_NUDGE_MARKER" ] && [ "$_QT" = "false" ]; then
  echo ""
  echo "gstack can learn from your AskUserQuestion answers. Run $plan --mode Discovery --module plan-tune to opt in"
...
`rm "${GSTACK_HOME:-$HOME/.gstack}"/.plan-tune-nudge-shown` before next ship.
...
<!-- GSTACK2_LEGACY_BODY_END source=ship -->
...
<!-- GSTACK2_BUG_FIX_START pr=884 anchor=GSTACK2_FIX_884_HUMAN_REVIEW_GATE -->
## Upstream judgment port: PR #884

[Treat requested human review as a hard landing gate](https://github.com/garrytan/gstack/pull/884)

### Human-review landing gate
