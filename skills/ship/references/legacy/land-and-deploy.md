...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for $ship --mode Land --module land-and-deploy is not yet implemented. Run `/ship` to create the MR, then merge manually via the GitLab web UI." Do not proceed.

# $ship --mode Land --module land-and-deploy — Merge, Deploy, Verify
...
## User-invocable
When the user types `$ship --mode Land --module land-and-deploy`, run this skill.

## Arguments
- `$ship --mode Land --module land-and-deploy` — auto-detect PR from current branch, no post-deploy URL
- `$ship --mode Land --module land-and-deploy <url>` — auto-detect PR, verify deploy at this URL
- `$ship --mode Land --module land-and-deploy #123` — specific PR number
- `$ship --mode Land --module land-and-deploy #123 <url>` — specific PR + verification URL
...
the ones listed below. The user said `$ship --mode Land --module land-and-deploy` which means DO IT — but verify
readiness first.
...
gh auth status
```
If not authenticated, **STOP**: "I need GitHub CLI access to merge your PR. Run `gh auth login` to connect, then try `$ship --mode Land --module land-and-deploy` again."
...
Check whether this project has been through a successful `$ship --mode Land --module land-and-deploy` before,
and whether the deploy configuration has changed since then:

```bash
eval "$($GSTACK_BIN/gstack-slug 2>/dev/null)"
if [ ! -f "${GSTACK_HOME:-$HOME/.gstack}"/projects/${PROJECT_ID:-unknown}/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat "${GSTACK_HOME:-$HOME/.gstack}"/projects/${PROJECT_ID:-unknown}/land-deploy-confirmed 2>/dev/null)
...
**If FIRST_RUN:** This is the first time `$ship --mode Land --module land-and-deploy` is running for this project. Before doing anything irreversible, show the user exactly what will happen. This is a dry run — explain, validate, and confirm.
...
If you want to persist deploy settings for future runs, suggest the user run `$ship --mode Deploy --module setup-deploy`.
...
- **RECOMMENDATION:** Choose A if all validations passed. Choose B if there are issues to fix. Choose C to run $ship --mode Deploy --module setup-deploy for a more thorough configuration.
...
- C) I want to configure this more carefully first (runs $ship --mode Deploy --module setup-deploy) (Completeness: 10/10)

**If A:** Tell the user: "Great — I've saved this configuration. Next time you run `$ship --mode Land --module land-and-deploy`, I'll skip the dry run and go straight to readiness checks. If your deploy setup changes (new platform, different workflows, updated URLs), I'll automatically re-run the dry run to make sure I still have it right."
...
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > "${GSTACK_HOME:-$HOME/.gstack}"/projects/${PROJECT_ID:-unknown}/land-deploy-confirmed
```
Continue to Step 2.

**If B:** **STOP.** "Tell me what's different about your setup and I'll adjust. You can also run `$ship --mode Deploy --module setup-deploy` to walk through the full configuration."

**If C:** **STOP.** "Running `$ship --mode Deploy --module setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to CLAUDE.md so I'll know exactly what to do next time. Run `$ship --mode Land --module land-and-deploy` again when that's done."
...
```
If `CONFLICTING`: **STOP.** "This PR has merge conflicts with the base branch. Resolve the conflicts and push, then run `$ship --mode Land --module land-and-deploy` again."
...
3. If drift is detected (a PR landed ahead of us and `BRANCH_VERSION < NEXT_SLOT`): **STOP** and print exactly:
   ...
   Exit non-zero. Do NOT auto-bump from `$ship --mode Land --module land-and-deploy` — rerunning `/ship` is the clean path (it already handles VERSION + package.json + CHANGELOG header + PR title atomically via Step 12 ALREADY_BUMPED detection).
...
```bash
cat references/artifacts/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
and tell the user: "I found and fixed a few issues during the review. The fixes are committed — run `$ship --mode Land --module land-and-deploy` again to pick them up and continue where we left off."

**If no issues found:** Tell the user: "Review checklist passed — no issues found in the diff."

**If B:** **STOP.** "Good call — run `/review` for a thorough pre-landing review. When that's done, run `$ship --mode Land --module land-and-deploy` again and I'll pick up right where we left off."
...
new features (new files, new commands, new skills): **WARNING — $ship --mode Prepare --module document-release
...
- If reviews are stale: "Run `/review` or `$plan --mode Full chain --module autoplan` to review the current code, then `$ship --mode Land --module land-and-deploy` again."
- If E2E not run: "Run your E2E tests to make sure nothing is broken, then come back."
- If docs not updated: "Run `$ship --mode Prepare --module document-release` to update CHANGELOG and docs."
...
Then tell the user: "Staging looks good. When you're ready for production, run `$ship --mode Land --module land-and-deploy` again."
**STOP.** The user can re-run `$ship --mode Land --module land-and-deploy` later for production.
...
<!-- GSTACK2_LEGACY_BODY_END source=land-and-deploy -->
