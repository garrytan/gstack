...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"document-generate","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> "${GSTACK_HOME:-$HOME/.gstack}"/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  $GSTACK_BIN/gstack-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
...
<!-- GSTACK2_LEGACY_BODY_END source=document-generate -->
