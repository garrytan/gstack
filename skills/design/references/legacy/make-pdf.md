...
empty boxes (▯). the explicitly approved `pdf` runtime capability attempts to install `fonts-noto-color-emoji` on Linux
...
excalidraw.com saves). Authoring NEW diagrams from English is `$design --mode Generate --module diagram`'s
