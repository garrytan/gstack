...
```
# $design --mode Implement --module design-review: Design Audit → Fix → Verify
...
"Your working tree has uncommitted changes. $design --mode Implement --module design-review needs a clean tree so each design fix gets its own atomic commit."
...
REPORT_DIR="${GSTACK_HOME:-$HOME/.gstack}/projects/${PROJECT_ID:-unknown}/designs/design-audit-$(date +%Y%m%d)"
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. AskUserQuestion: "This site requires authentication. Want to import cookies from your browser? Run `$qa --mode Report --module setup-browser-cookies` first if needed."
...
```
Write to: `"${GSTACK_HOME:-$HOME/.gstack}"/projects/<stable-project-id>/{user}-{branch}-design-audit-{datetime}.md`
...
```
"${GSTACK_HOME:-$HOME/.gstack}"/projects/${PROJECT_ID:-unknown}/designs/design-audit-{YYYYMMDD}/
...
For CSS-only fixes: skip entirely. CSS regressions are caught by re-running $design --mode Implement --module design-review.
...
eval "$($GSTACK_BIN/gstack-slug 2>/dev/null)" && mkdir -p "${GSTACK_HOME:-$HOME/.gstack}"/projects/${PROJECT_ID:-unknown}
```
Write a one-line summary to `"${GSTACK_HOME:-$HOME/.gstack}"/projects/<stable-project-id>/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
2. **Fixed findings that were in TODOS.md** → annotate with "Fixed by $design --mode Implement --module design-review on {branch}, {date}"
...
<!-- GSTACK2_LEGACY_BODY_END source=design-review -->
