...
Be specific. Reference file:line for every finding." -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' $(~/.claude/skills/gstack/bin/gstack-codex-search-flag) < /dev/null 2>"$TMPERR_DESIGN"
```
