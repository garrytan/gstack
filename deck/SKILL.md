...
available user-question mechanism for one Step 1 intake round. If product use is
pending, include the proposed Access policy in that same round; after the reply,
resume Step 0 as above before Step 2. In Conductor, or when that mechanism is
unavailable, render the same labelled questions as concise prose and stop for
the reply. A spawned
...
After showing the inferred brief, ask one consolidated intake round with at most
three questions. Each question must name its material category, the current
...
non-blockers become unknown, qualified, or omitted. Reopen intake only when the
user's answer creates a contradiction in one of the seven categories. A question
may pair adjacent categories, but it must ask for one decision or one tightly
related evidence bundle; never hide a long questionnaire inside one item.
...
`Source material` permits only asking the user to point to or provide the single
highest-leverage source/evidence bundle, identify its authoritative owner, or
choose qualification or omission. Do not relabel product strategy, visual taste,
scope, or implementation choices as source-material questions.
...
inspection plus the bounded Source-material response cannot establish them,
...
child's mandatory prompt never overrides Step 1. Return to Step 1 only for a new
contradiction in one of its seven material categories.
