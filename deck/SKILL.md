...
name: deck
version: 1.0.0
description: Add or redesign an interactive web deck on an existing site. (gstack)
triggers:
...
It starts
with the site, story, design system, routing, hosting, and evidence;
...
category. It is off by default. A legacy PDF discovered during inspection is
source material, not a delivery choice: it activates companion work only when
the user explicitly selects it as a deliverable or explicitly supplies a
requested companion. When selected, settle its intended delivery location and
access policy with Route / host; record the final artifact name/direct URL only
after the actual build. Do not spend a separate intake question on
implementation. When unselected, add no deck-PDF link, generator, route, build
dependency, or PDF generation, rendering, or visual QA.
...
truth map, route or host and PDF decision (off = no artifact, canonical source,
or generator; on = intended delivery location, access policy, canonical source,
and generator), access boundary, section order,
and a claim ledger. For
...
no deck-PDF link, generator, route, build dependency, or PDF generation,
rendering, or visual QA.

Generate it from the same approved story, claim ledger, heading order, and
access policy as the web deck. It may use a narrower compatible boundary only
when the user expressly selects it; it must never be broader. A format-specific
publication view is allowed only when the preservation ledger records where each
substantive point, citation, qualifier, and required decision content appears;
never keep a silent second narrative. Use the target project's existing
print/build/render path.
...
7. If a PDF companion was selected: canonical-source provenance; a non-empty PDF
   ...
   hierarchy, readable type, clipping, page splits, citations/footnotes, and
   missing or remote assets. Use available
   ...
   it.
8. If no PDF companion was selected: perform the normal release-diff
   confirmation that no deck-PDF artifact, link, generator, or build dependency
   was added. This is not PDF generation, rendering, or visual QA.
