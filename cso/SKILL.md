...
7. **Redaction / masking modes:** A flag that promises to strip secrets, amounts, or identifiers from an output is PROVEN by running it, never by reading it. Build a malformed and an out-of-vocabulary input in a scratch directory (unrecognized category value, wrong column count, truncated row, wrong type), run the masked command against it, and grep the output for the raw values. The leak lives in the error, exception-detail and fallback branches — the paths a well-formed sample never reaches and a code read skips because the happy path is clean. Synthetic input only, run locally against the audited code: never a real dataset, never a live system, and never edit the code to test it.
