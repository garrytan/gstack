...
DEVICE=$(printenv COMPUTERNAME 2>/dev/null || hostname 2>/dev/null || echo "unknown")
# Prefer repo-local .gstack/designs/ for cross-device portability (sync via git + gstack-publish)
REPO_TOP=$(git rev-parse --show-toplevel 2>/dev/null || echo "")
if [ -n "$REPO_TOP" ]; then
  DESIGNS_DIR="$REPO_TOP/.gstack/designs"
  mkdir -p "$DESIGNS_DIR"
  DESIGN_PATH="$DESIGNS_DIR/$USER-$BRANCH-design-$DATETIME.md"
else
  mkdir -p ~/.gstack/projects/$SLUG
  DESIGN_PATH="$HOME/.gstack/projects/$SLUG/$USER-$BRANCH-design-$DATETIME.md"
  echo "WARNING: Not inside a git repo — design doc written to ~/.gstack/projects/ and will not sync automatically. Run gstack-publish to get a URL, or run this session from inside a repo."
fi
```
...
if [ -n "$REPO_TOP" ] && [ -d "$REPO_TOP/.gstack/designs" ]; then
  PRIOR=$(find "$REPO_TOP/.gstack/designs" -name "*-$BRANCH-design-*.md" 2>/dev/null | sort -r | head -1)
fi
[ -z "$PRIOR" ] && PRIOR=$(find ~/.gstack/projects/$SLUG -maxdepth 1 -name "*-$BRANCH-design-*.md" 2>/dev/null | sort -r | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `$DESIGN_PATH` (repo-local `.gstack/designs/` when inside a git repo; `~/.gstack/projects/{slug}/` otherwise).

After writing the design doc, tell the user:
**"Design doc saved to: {full path}. Run `gstack-publish` to approve and sync cross-device. Other skills (/plan-ceo-review, /plan-eng-review) will find it automatically."**

### Startup mode design doc template:

```markdown
---
source_device: {DEVICE — $COMPUTERNAME on Windows, $(hostname) on Mac}
date: {YYYY-MM-DD}
status: DRAFT
plan_url: (populated by gstack-publish)
---
# Design: {title}
