...
Present the reviewed design doc to the user via AskUserQuestion. If AskUserQuestion is unavailable, use `D<N>` for this discrete approval decision; use `Q<N>` for open-ended questions with no options list:
...
Then use AskUserQuestion: "Would you consider applying to Y Combinator?" If AskUserQuestion is unavailable, use `D<N>` for this yes/no decision; use `Q<N>` for open-ended questions with no options list.
...
3. Use AskUserQuestion to offer opening the resources. If AskUserQuestion is unavailable, use `D<N>` for this resource-opening decision; use `Q<N>` for open-ended questions with no options list:
...
**Otherwise**, offer via AskUserQuestion (D<N> format from the preamble for this discrete decision; use Q<N> for open-ended questions with no options list):
