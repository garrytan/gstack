...
description: |
  ...
  "let another agent use my browser", or "give browser access". (jstack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"pair-agent","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.jstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/jstack/bin/jstack-timeline-log '{"skill":"pair-agent","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
Your jstack browser runs a local HTTP server. This skill creates a one-time setup key,
...
side panel if you have JStack Browser open."
...
credentials are written to `~/.openclaw/skills/jstack/browse-remote.json`.
...
to `~/.codex/skills/jstack/browse-remote.json`.
...
`~/.cursor/skills/jstack/browse-remote.json`.
