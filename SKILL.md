...
2. Run this read-only history preflight before building the menu:
   ...
   - Reduce the outcome to three to eight lowercase keywords containing only
     ...
     keywords as one quoted argument to
     `~/.claude/skills/gstack/bin/gstack-decision-search --query "$SAFE_TERMS" --semantic`.
     The `--semantic` path owns optional GBrain recall and confines it to the
     curated-memory source; it degrades to local decisions when GBrain is absent,
