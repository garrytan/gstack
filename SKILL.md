...
1. If the request is about a browser, QA, dogfooding, screenshots, or inspecting a page
   ...
   `/benchmark`, `/scrape`) drives the Aside browser — the user's real browser with their
   real logged-in sessions — so there is no separate "open the browser" or "import cookies"
   step to route to.
