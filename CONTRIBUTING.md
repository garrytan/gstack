...
```bash
# 需要在 .env 里配置 ANTHROPIC_API_KEY —— bun run test:evals 会自动用到
```
...
### 第 2 步：运行 setup，创建每个 skill 的 symlink
