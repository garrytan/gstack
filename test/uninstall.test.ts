...
describe('gstack-uninstall', () => {
  ...
  describe('integration tests with mock layout', () => {
    ...
    beforeEach(() => {
      ...
      // Create a managed PATH shim that uninstall should clean up.
      fs.mkdirSync(path.join(mockHome, '.local', 'bin'), { recursive: true });
      fs.writeFileSync(
        path.join(mockHome, '.local', 'bin', 'gstack-autopilot'),
        '#!/usr/bin/env bash\n# gstack-managed autopilot shim. Generated by ./setup.\nexit 0\n',
      );
    ...
    test('--force removes global Claude skills and state', () => {
      ...
      // Managed PATH shim should be removed
      expect(fs.existsSync(path.join(mockHome, '.local', 'bin', 'gstack-autopilot'))).toBe(false);
