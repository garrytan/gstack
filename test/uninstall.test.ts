...
describe('gstack-uninstall', () => {
  ...
  describe('integration tests with mock layout', () => {
    ...
    beforeEach(() => {
      ...
      fs.mkdirSync(path.join(mockHome, '.antigravity', 'skills', 'gstack'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.antigravity', 'skills', 'gstack', 'SKILL.md'), 'test');

      // Create per-skill symlinks (both old unprefixed and new prefixed)
      fs.symlinkSync('gstack/review', path.join(mockHome, '.antigravity', 'skills', 'review'));
      fs.symlinkSync('gstack/ship', path.join(mockHome, '.antigravity', 'skills', 'gstack-ship'));

      // Create a non-gstack symlink (should NOT be removed)
      fs.mkdirSync(path.join(mockHome, '.antigravity', 'skills', 'other-tool'), { recursive: true });
    ...
    test('--force removes global Antigravity skills and state', () => {
      const result = spawnSync('bash', [UNINSTALL, '--force'], {
        stdio: 'pipe',
        env: {
          ...process.env,
          HOME: mockHome,
          GSTACK_DIR: path.join(mockHome, '.antigravity', 'skills', 'gstack'),
      ...
      expect(fs.existsSync(path.join(mockHome, '.antigravity', 'skills', 'gstack'))).toBe(false);

      // Per-skill symlinks pointing into gstack/ should be removed
      expect(fs.existsSync(path.join(mockHome, '.antigravity', 'skills', 'review'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.antigravity', 'skills', 'gstack-ship'))).toBe(false);

      // Non-gstack tool should still exist
      expect(fs.existsSync(path.join(mockHome, '.antigravity', 'skills', 'other-tool'))).toBe(true);
