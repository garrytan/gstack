  ...
  try {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      if (signal?.outcome === 'permission_dialog') {
        session.send(`${defaultPick}`);
        await Bun.sleep(1500);
        continue;
      }

      // Once the Step-0 boundary has fired, any non-permission numbered or
      // prose-rendered AUQ means the review-phase floor has been met. The
      // prose path covers hosts where AskUserQuestion is disallowed and the
      // model renders options directly in text.
      ...
      // LLM judge fallback: after 60s of polling without a regex hit, ask the
      // classifier to detect prose-style waiting states. Only count this after
      // the Step-0 boundary so setup questions do not satisfy the review floor.
      const elapsed = Date.now() - start;
      if (
        boundaryFired &&
