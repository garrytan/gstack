...
export const CARVE_GUARDS: Record<string, CarveGuard> = {
  ...
  'plan-ceo-review': {
    ...
    maxSkeletonBytes: 94_800, // fork: +single-select AskUserQuestion doctrine (~440B) on the v1.68 #2402 base; measured 94,325
  ...
  'plan-eng-review': {
    ...
    maxSkeletonBytes: 72_700, // fork: +single-select AskUserQuestion doctrine (~440B); measured 72,208
  ...
  'plan-design-review': {
    ...
    maxSkeletonBytes: 92_600, // fork: +single-select AskUserQuestion doctrine (~440B); measured 92,156
    minUnionBytes: 70_000,
  ...
  'plan-devex-review': {
    ...
    // #2499 project-scope MCP jq in the brain-sync block grew every tier-2+
    // skeleton ~1.5KB (entry resolution emitted once per SKILL.md).
    maxSkeletonBytes: 84_400, // fork: +single-select AskUserQuestion doctrine (~440B); measured 83,952
    minUnionBytes: 70_000,
  ...
  'office-hours': {
    ...
    // #2499 project-scope MCP jq in the brain-sync block grew every tier-2+
    // skeleton ~1.5KB (entry resolution emitted once per SKILL.md).
    maxSkeletonBytes: 103_700, // fork: +single-select AskUserQuestion doctrine (~440B); measured 103,200
    minUnionBytes: 70_000,
  ...
  'document-release': {
    ...
    // always-loaded AskUserQuestion Format section.
    // v1.2.0 activation lift: first-run-guidance section in the shared preamble.
    maxSkeletonBytes: 58_900, // fork: +single-select AskUserQuestion doctrine (~440B); measured 58,396
  ...
  'design-consultation': {
    ...
    maxSkeletonBytes: 72_300, // fork: +single-select AskUserQuestion doctrine (~440B); measured 71,795
  ...
  cso: {
    ...
    // always-loaded AskUserQuestion Format section.
    // v1.2.0 activation lift: first-run-guidance section in the shared preamble.
    maxSkeletonBytes: 78_200, // fork: +single-select AskUserQuestion doctrine (~440B); measured 77,716
    minUnionBytes: 72_000,
