...
test('Eng loads its one remedy procedure before Scope Challenge findings and retains consent', async () => {
  ...
  for (const suffix of ['.tmpl', '']) {
    ...
    expect(compactProse(sections)).toContain('Select the report file and permissions under **Review record and write policy**');
    ...
    expect(selfCheck).toContain('If evidence is missing, reread the rendered review-sections section above and use Recovery routing');
...
describe('Eng approved-work decision gate', () => {
  ...
  test('reconciles operative decision State before approval readiness and unresolved counts', () => {
    ...
    expect(compactProse(outputs)).toContain("For recovery or changed outputs, use the entrypoint's **Recovery routing**: repeat affected output steps 1–4");
    expect(compactProse(outputs)).toContain('reuse a successful Review Log only for unchanged saved outputs');
  ...
  test('approval readiness follows TODO decisions and finalization returns forward exactly once', () => {
    ...
    expect(compactProse(closing)).toContain("entrypoint's Section self-check and read-only EXIT PLAN MODE GATE. Run both in every host mode");
