              ...
              const result = await runAgentSdkTest({
                ...
                userPrompt: `Load gstack's ${name.startsWith('commands-') ? '/ship and /land-and-deploy validation' : '/land-and-deploy readiness'} workflow excerpt at ${fixture.skillPath} and execute it in this isolated repository. ` +
                  (name.startsWith('commands-')
                    ? 'Platform/base detection is complete: GitHub, base ref origin/main. Step 4 discovery is ready: read repository instructions and CI. Execute the supplied ship Steps 5, 6 and 16, then the supplied landing test-results gate using the ship handoff. Use the supplied evidence helper as provided; fixture control files, harness event logs and helper implementation are outside this task. Validation result logs remain in scope. No other ship/land phases are in scope. Stop if a required gate cannot be resolved.'
