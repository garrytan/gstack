...
import { SHIP_LAND_CASES } from './helpers/ship-land-fixture';
import { ENROLLMENT_CASES } from './helpers/transcript-enrollment-fixture';
import {
...
const expectedWalls = {
  ...
  'test/skill-e2e-ship-land-contracts.test.ts': 7_660_000,
  'test/skill-e2e-transcript-enrollment.test.ts': 3_840_000,
...
test('added workflow callbacks retain their complete case sets and existing attempt limits', () => {
  expect(SHIP_LAND_CASES).toHaveLength(29);
  expect(new Set(SHIP_LAND_CASES).size).toBe(29);
  expect(SHIP_LAND_CASES.filter(name => name.startsWith('commands-'))).toHaveLength(8);
  expect(SHIP_LAND_CASES.filter(name => name.startsWith('review-'))).toHaveLength(16);
  expect(SHIP_LAND_CASES.filter(name => name.startsWith('ci-'))).toHaveLength(5);
  expect(ENROLLMENT_CASES).toEqual([
    { choice: 'A', count: 2 }, { choice: 'B', count: 2 }, { choice: 'C', count: 2 },
    { choice: 'D', count: 2 }, { choice: 'E', count: 2 }, { choice: 'E', count: 0 },
  ]);
  for (const [file, cases, budgetMs, loop, timeout] of [
    ['test/skill-e2e-ship-land-contracts.test.ts', SHIP_LAND_CASES.length, 120_000,
      'for (const name of SHIP_LAND_CASES)', 'BUDGET_MS + OFFICE_HOURS_BUN_GRACE_MS'],
    ['test/skill-e2e-transcript-enrollment.test.ts', ENROLLMENT_CASES.length, ALL_TIERS.CAPTURE_MS,
      'for (const { choice, count } of ENROLLMENT_CASES)', 'CAPTURE_MS + OFFICE_HOURS_BUN_GRACE_MS'],
  ] as const) {
    const source = read(file);
    const row = FILE_RETRY_BUDGETS.find(row => row.file === file)!;
    expect(source).toContain(loop);
    expect(source).toContain(`}, ${timeout});`);
    expect(row.retries).toBe(1);
    expect(retriesForFiles([file])).toBe(1);
    expect(row.shardMs).toBe(cases * (budgetMs + 10_000) * 2 + 120_000);
  }
  expect(read('test/skill-e2e-ship-land-contracts.test.ts')).toContain('const BUDGET_MS = 120_000');
  expect(read('test/skill-e2e-ship-land-contracts.test.ts')).toContain('budgetMs: BUDGET_MS');
  expect(read('test/skill-e2e-transcript-enrollment.test.ts')).toContain('budgetMs: CAPTURE_MS');
  const render = read('test/skill-e2e-setup-deploy-render.test.ts');
  expect(render.match(/\['setup-deploy-render-(?:unset|empty|present)',/g)).toHaveLength(3);
  expect(render).toContain('for (const [testName, value] of cases)');
  expect(render).toContain('timeout: CAPTURE_MS - 60_000');
  expect(render).toContain('}, CAPTURE_MS - 30_000);');
  expect(retriesForFiles(['test/skill-e2e-setup-deploy-render.test.ts'])).toBe(1);
  expect(resolvePaidShardBudget(['test/skill-e2e-setup-deploy-render.test.ts']).timeoutMs)
    .toBeGreaterThanOrEqual(3 * (ALL_TIERS.CAPTURE_MS - 30_000) * 2 + 120_000);
});
...
test('both gate executors cover the complete census without increasing aggregate workers', () => {
  ...
  for (const [workflow, jobName, workers, slices] of [[main, 'eval-slices', 2, 6], [periodic, 'gate-census', 1, 7]] as const) {
    ...
    expect(manifest.entries.filter(row => row.status === 'planned')).toHaveLength(57);
    ...
    expect(executor['timeout-minutes']).toBeLessThanOrEqual(360);
