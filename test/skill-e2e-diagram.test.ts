/**
 * /diagram skill E2E (paid, claude -p) — periodic tier, live Aside required.
 *
 * gstack renders local HTML through the Aside browser: the /diagram skill runs
 * ONE `bun run bin/gstack-render.ts` call per triplet (staged bundle, three
 * --eval/--out pairs). Aside is an external service with no CI runner
 * (CLAUDE.md tiering rule 3), so the whole file is periodic and self-skips —
 * never fails — when EVALS_TIER is not 'periodic' or `asideAvailable()` is
 * false (no `aside` on PATH, app not running, or GSTACK_SKIP_ASIDE=1):
 *
 *   EVALS=1 EVALS_TIER=periodic EVALS_ALL=1 bun test test/skill-e2e-diagram.test.ts
 *
 * Two tests:
 *
 *   diagram-triplet — deterministic functional contract: from an English ask,
 *   the agent following the skill emits a parseable triplet (.mmd source,
 *   .excalidraw scene with elements, SVG markup, PNG bytes) and did it through
 *   gstack-render (a Bash tool call names it). No quality judgment.
 *
 *   diagram-authoring-quality — LLM-judged benchmark of the authored mermaid
 *   itself (faithfulness to the ask, label quality, readable size).
 *
...
if (process.env.EVALS && !shouldRun) {
  process.stderr.write(`\nskill-e2e-diagram: SKIPPED — ${tierOk
...
- Aside is installed and running on this machine.
  ...
  testConcurrentIfSelected('diagram-triplet', async () => {
    ...
    try {
      const result = await runSkillTest({
        prompt: basePrompt(
          dir,
          'a flowchart (graph LR) of a 4-stage pipeline: markdown → prepass → Aside render → PDF.',
        ),
  ...
  testConcurrentIfSelected('diagram-authoring-quality', async () => {
    ...
    try {
      const result = await runSkillTest({
        prompt: basePrompt(
          dir,
          'how gstack renders diagrams in PDFs: markdown containing mermaid fences goes through a pre-pass that extracts the fences, renders them in the Aside browser using an offline bundle, substitutes the SVG back in, inlines local images, and prints the PDF through Aside. Failures become visible diagnostic blocks.',
        ),
        workingDirectory: dir,
        maxTurns: 25,
        allowedTools: ['Bash', 'Read', 'Write'],
...
extracted by a pre-pass, rendered in the Aside browser via an offline bundle,
substituted back as SVG, images inlined, printed to PDF through Aside, with
render failures becoming visible diagnostic blocks.
