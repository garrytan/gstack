...
describe('workflow judge excerpts', () => {
  ...
  test('ship uses project-native commands and never jumps over mandatory gates', () => {
    ...
    expect(text).toContain('$GSTACK_ANCHOR_INVOCATION gstack-version-bump classify');
    expect(text).toContain('RELEASE_ASSERTIONS=(--lane "$ECPE_EXECUTION_LANE"');
    expect(text).toContain('When `RELEASE_APPLICABLE=false`');
    expect(text).not.toContain('gstack-next-version');
  ...
  test('ship publishes existing PRs only after shared body composition and scan', () => {
    ...
    const edit = publish.indexOf('PROVIDER_PR_JSON=$(');
    expect(compose).toBeGreaterThan(0);
    expect(scan).toBeGreaterThan(compose);
    expect(edit).toBeGreaterThan(scan);
    expect(publish.slice(edit)).toContain('gstack-effect-scope provider-pr');
    expect(publish.slice(edit)).toContain('--body-file "$PR_BODY_FILE" --assert-body-sha256 "$PR_BODY_SHA256"');
    expect(publish.slice(0, edit)).toContain('PROVIDER_PR_ACTION_ARGS=(update --pr "$PR_NUMBER")');
    expect(publish.slice(0, edit)).not.toMatch(/(?:continue|proceed|go) to Step 20/i);
    expect(publish.indexOf('**Output the PR/MR URL**')).toBeGreaterThan(edit);
    expect(publish).not.toMatch(/\b(?:gh pr|glab mr) (?:create|edit)\b/);
