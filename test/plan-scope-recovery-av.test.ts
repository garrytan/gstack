...
const recovery = (text: string) => text.split('\n').find(line => line.startsWith('> Before ') && line.includes('publicly named'))!;
...
test('the review handoff repairs a missing public declaration without claiming timely compliance', () => {
  for (const skill of skills) {
    ...
    expect(check).toContain('For plan-mode auto-selection, verify you publicly named the selected plan after skill load');
...
test('unseeded, explicit-target and early announcement rules remain authoritative on every host', () => {
  for (const skill of skills) {
    ...
    expect(gate).toContain('Announce plan-mode auto-selection before tools');
    ...
    for (const host of ALL_HOST_CONFIGS) {
      ...
      expect(expanded.indexOf('Announce plan-mode auto-selection before tools')).toBeLessThan(expanded.indexOf('```bash'));
...
test('all four exact public failures remain failures; introducing the skill is not target selection', () => {
  expect(observedFailures).toHaveLength(4);
  for (const row of observedFailures) {
