...
describe('Codex generation (--host codex)', () => {
  ...
  test('review asset paths resolve through $GSTACK_ROOT (not a repo-only path)', () => {
    // Regression: generated Codex skills hard-coded .agents/skills/gstack/review,
    // which fails when Codex falls back to the global runtime root.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gstack-review', 'SKILL.md'), 'utf-8');
  ...
  test('review asset paths in ship resolve through $GSTACK_ROOT', () => {
