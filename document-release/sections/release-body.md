...
source $GSTACK_BIN/gstack-codex-probe 2>/dev/null || true
...
**Persist the result:**
```bash
$GSTACK_BIN/gstack-review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
