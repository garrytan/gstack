...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"plan-status","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
- If multiple plans exist with no clear match, use AskUserQuestion to let the user pick.
