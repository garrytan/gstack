...
### A. Inspect the proposed scope

Collect evidence for the complexity gate and findings below:
...
- **Complexity check:** Count files and new classes/services; seek fewer moving parts.
...
### B. Resolve the complexity gate

At 8+ files or 2+ new classes/services, STOP before Section 1 and follow B1–B3.
Below both thresholds, skip B1–B3 and continue to **C. Resolve scope findings**.
...
Unlike the remedy procedure, ask them without first saving a pending record or
comparison grid. Wait for actual answers before changing scope.

1. **B1 — Feature scope.** Explain the complexity. Ask each proposed feature cut/deferral separately;
   wait before changing scope. With no proposed cuts, keep the feature list and
   go directly to the structure question.
2. **B2 — Structure.** Always ask the structure question when this gate trips, even with no cuts.
...
3. **B3 — Record and apply.** Save the actual feature and structure answers as one scope record: `feature
   answers: <refs>; structure: <A/B + ref>; accepted scope: <exact scope>;
   pending remedies: <ids or none>`.

   Save this record under the write policy after the answers arrive. Do not create
   a backdated pending brief or claim it was saved before asking. Apply only
   accepted scope changes; do not re-argue reduction or skip approved components.
   Continue to **C. Resolve scope findings**.

### C. Resolve scope findings
