...
| `GET /activity/stream` | Bearer OR view-only SSE token | 404 | SSE. ?token= query param no longer accepted |
| `GET /inspector/events` | Bearer OR view-only SSE token | 404 | SSE. Same token as /activity/stream |
| `POST /sse-session` | auth (Bearer) | 404 | Mints view-only 30-min SSE token |
...
**SSE session tokens.** EventSource can't send Authorization headers, so the extension POSTs `/sse-session` once at bootstrap with the root Bearer and receives a 30-minute view-only token. Browser pages can use the `gstack_sse` HttpOnly SameSite=Strict cookie; Chrome-extension pages use scoped `?sse=<VIEW_ONLY>` because SameSite cookies are not reliable from extension origins. The token is valid ONLY for `/activity/stream` and `/inspector/events` — it is NOT a scoped command token and cannot be used on `/command`. Scope isolation is enforced by the module boundary: `sse-session-cookie.ts` has no imports from `token-registry.ts`.
