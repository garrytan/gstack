...
ports). Authenticate one session per role:

```bash
# One token per actor role; never hardcode credentials in the report — [REDACTED]
curl -s -X POST "$TARGET/auth/login" -d '{"username":"<role-account>","password":"[REDACTED]"}'
```
...
Per matrix row: log in as the actor, navigate to the entity, attempt the
transition through the UI, screenshot before/after into
