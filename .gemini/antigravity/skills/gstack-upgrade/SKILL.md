...
[ -z "$_AUTO" ] && _AUTO=$(~/.gemini/antigravity/skills/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.gemini/antigravity/skills/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.gemini/antigravity/skills/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.gemini/antigravity/skills/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.gemini/antigravity/skills/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gemini/antigravity/skills"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".gemini/antigravity/skills/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".gemini/antigravity/skills"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".gemini/antigravity/skills" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.gemini/antigravity/skills" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.gemini/antigravity/skills" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.gemini/antigravity/skills"
  fi
fi
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.gemini/antigravity/skills/` when you're ready."
...
```bash
~/.gemini/antigravity/skills/bin/gstack-update-check --force 2>/dev/null || \
.gemini/antigravity/skills/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.gemini/antigravity/skills/` when you're ready."
