...
GSTACK_ROOT="$HOME/undefined/skills/gstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.gemini/antigravity/skills" ] && GSTACK_ROOT="$_ROOT/.gemini/antigravity/skills"
...
~/.gemini/antigravity/skills/bin/gstack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
eval "$(~/.gemini/antigravity/skills/bin/gstack-slug 2>/dev/null)"
...
~/.gemini/antigravity/skills/bin/gstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x ~/.gemini/antigravity/skills/bin/gstack-telemetry-log ]; then
  ~/.gemini/antigravity/skills/bin/gstack-telemetry-log \
...
\`\`\`bash
~/.gemini/antigravity/skills/bin/gstack-review-read
\`\`\`
...
For Design Review: run `source <(~/.gemini/antigravity/skills/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.gemini/antigravity/skills/gstack/review/TODOS-format.md`).
...
```bash
_CROSS_PROJ=$(~/.gemini/antigravity/skills/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.gemini/antigravity/skills/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.gemini/antigravity/skills/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `~/.gemini/antigravity/skills/bin/gstack-config set cross_project_learnings true`
If B: run `~/.gemini/antigravity/skills/bin/gstack-config set cross_project_learnings false`
...
1. Read `.gemini/antigravity/skills/gstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
source <(~/.gemini/antigravity/skills/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.gemini/antigravity/skills/gstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.gemini/antigravity/skills/bin/gstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
~/.gemini/antigravity/skills/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
Read `.gemini/antigravity/skills/gstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
OLD_CFG=$(~/.gemini/antigravity/skills/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.gemini/antigravity/skills/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.gemini/antigravity/skills/bin/gstack-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
Read `.gemini/antigravity/skills/gstack/review/TODOS-format.md` for the canonical format reference.
