...
~/.gemini/antigravity/skills/bin/gstack-timeline-log '{"skill":"land-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.gemini/antigravity/skills/bin/gstack-review-read 2>/dev/null
```
...
```bash
cat ~/.gemini/antigravity/skills/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
eval $(~/.gemini/antigravity/skills/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
