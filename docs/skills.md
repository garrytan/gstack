...
| [`/fanout`](#fanout) | **Multi-agent Dispatcher** | Take a finished design doc and decompose it into N parallel agent tasks. Identifies Slab 0 plus 2-3 independent slabs, appends a Parallel Execution Plan section to the doc, and emits a `worktree-dispatch.sh` sidecar. v0 produces the plan and stops; user runs the script. |
