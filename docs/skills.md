...
| [`/automate`](#automate) | **Browser Flow Automator** | Automate multi-step mutating flows on a page. First call prototypes via `$B` with strict per-step user confirmation gates; subsequent calls run codified in ~200ms. |
| [`/skillify`](#skillify) | **Skill Codifier** | Walks back through your conversation, finds the last `/scrape` or `/automate` prototype, synthesizes script + test + fixture, runs the test, asks before committing. |
