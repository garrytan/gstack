...
| [`/sync-gbrain`](#sync-gbrain) | **Keep Brain Current** | Sync this repo's changed Markdown and code to gbrain; teach the agent when to use `gbrain search`/`code-def` over Grep. Idempotent; safe to re-run. |
| | | |
| **Safety & Utility** | | |
