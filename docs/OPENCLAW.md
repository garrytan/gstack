# jstack x OpenClaw Integration

jstack integrates with OpenClaw as a methodology source, not a ported codebase.
OpenClaw's ACP runtime spawns Claude Code sessions natively. jstack provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │   investigate, retro                   │
       │                                        ├── Generates jstack-lite
       ├── sessions_spawn(runtime: "acp")       │   (planning discipline)
       │       │                                │
       │       └── Claude Code                  ├── Generates jstack-full
       │           └── jstack installed at      │   (complete pipeline)
       │               ~/.claude/skills/jstack  │
...
OpenClaw decides at spawn time which tier of jstack support to use:

| Tier | When | Prompt prefix |
|------|------|---------------|
| **Simple** | One-file edits, typos, config changes | No jstack context injected |
| **Medium** | Multi-file features, refactors | jstack-lite CLAUDE.md appended |
| **Heavy** | Specific jstack skill needed | "Load jstack. Run /X" |
| **Full** | Complete features, objectives, projects | jstack-full pipeline appended |
| **Plan** | "Help me plan a Claude Code project" | jstack-plan pipeline appended |
...
The complete ready-to-paste section lives in `openclaw/agents-jstack-section.md`.
Copy it into your OpenClaw AGENTS.md.

Key behavioral rules (these go ABOVE the dispatch tiers):

1. **Always spawn, never redirect.** When the user asks to use ANY jstack skill,
...
jstack-lite/full as a new section. Do not replace the repo's existing instructions.

## What jstack generates for OpenClaw

All artifacts live in the `openclaw/` directory and are generated by
`bun run gen:skill-docs --host openclaw`:

### jstack-lite (Medium tier)
`openclaw/jstack-lite-CLAUDE.md` — ~15 lines of planning discipline:
...
### jstack-full (Full tier)
`openclaw/jstack-full-CLAUDE.md` — chains existing jstack skills:
...
### jstack-plan (Plan tier)
`openclaw/jstack-plan-CLAUDE.md` — full review gauntlet, no implementation:
...
- `jstack-openclaw-office-hours` — Product interrogation (6 forcing questions)
- `jstack-openclaw-ceo-review` — Strategic challenge (10-section review, 4 modes)
- `jstack-openclaw-investigate` — Operational debugging (4-phase methodology)
- `jstack-openclaw-retro` — Operational retrospective (weekly review)

Source lives in `openclaw/skills/` in the jstack repo. These are hand-crafted
adaptations of the jstack methodology for OpenClaw's conversational context.
No jstack infrastructure (no browse, no telemetry, no preamble).

## Spawned session detection

When Claude Code runs inside a session spawned by OpenClaw, the `OPENCLAW_SESSION`
environment variable should be set. jstack detects this and adjusts:
...
For OpenClaw users: tell your OpenClaw agent "install jstack for openclaw."

The agent should:
1. Install jstack-lite CLAUDE.md into its coding session templates
2. Install the 4 native methodology skills
3. Add dispatch routing to AGENTS.md
4. Verify with a test spawn

For jstack developers: `./setup --host openclaw` outputs this documentation.
...
- No SOUL.md from jstack (OpenClaw has its own)
