...
SSE endpoints (`/activity/stream`, `/inspector/events`) accept either a Bearer token, the HttpOnly `gstack_sse` cookie, or a scoped `?sse=<VIEW_ONLY>` token minted via `POST /sse-session` (30-minute TTL, stream-scope only — cannot be used against `/command`). As of v1.6.0.0 the legacy `?token=<ROOT>` query-string auth is no longer accepted.
...
- **SSE auth** uses a 30-minute view-only token, delivered as an HttpOnly SameSite=Strict cookie for browser pages or scoped `?sse=` token for Chrome-extension EventSource (never valid against `/command`).
