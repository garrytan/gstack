# Remote Browser Access — How to Pair With a JStack Browser

A JStack Browser server can be shared with any AI agent that can make HTTP requests.
...
─────────────                         ────────────
JStack Browser Server                 Any AI agent
...
```bash
$B pair-agent --local openclaw    # writes to ~/.openclaw/skills/jstack/browse-remote.json
$B pair-agent --local codex       # writes to ~/.codex/skills/jstack/browse-remote.json
$B pair-agent --local cursor      # writes to ~/.cursor/skills/jstack/browse-remote.json
```
...
3. Save it: `echo 'NGROK_AUTHTOKEN=your_token' > ~/.jstack/ngrok.env`
4. Optionally claim a stable domain: `echo 'NGROK_DOMAIN=your-name.ngrok-free.dev' >> ~/.jstack/ngrok.env`
