...
How the JStack Browser sidebar actually works. Read this before touching
...
T+1-3s    Extension loads in Chromium
            ...
            │     └── Does NOT fire jstack-extension-ready (waits for sidebar)
...
4. server.ts /sidebar-command handler
   ...
   └── Appends JSON to ~/.jstack/sidebar-agent-queue.jsonl
...
4. welcome.html script
   └── addEventListener('jstack-extension-ready', () => arrow.classList.add('hidden'))
```
...
Server starts → AUTH_TOKEN = crypto.randomUUID()
    ...
    │
    └── sidebar-agent.ts refreshToken() → reads from .jstack/browse.json
...
| Agent stale token | Agent runs but no events appear in sidebar | sidebar-agent has old token from .jstack/browse.json | Agent re-reads token before each event POST |
...
| Queue file | `~/.jstack/sidebar-agent-queue.jsonl` | Filesystem |
| State file | `.jstack/browse.json` | Filesystem |
| Chat log | `~/.jstack/sessions/<id>/chat.jsonl` | Filesystem |
