...
The full style and versioning bible for gstack releases. CLAUDE.md keeps a short
summary of the load-bearing invariants and points here for the detail. The
operational ship-time checklist (enumerate commits → group → write → cross-check)
lives in the `/ship` skill (`generateChangelogWorkflow` resolver), not here — this
doc is the rules and voice, not the procedure.
