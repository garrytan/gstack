...
describe('bun-polyfill', () => {
  ...
  test('Bun.sleep resolves after delay', async () => {
    const result = Bun.spawnSync([nodePath, '-e', `
  ...
  test('Bun.spawnSync runs a command and returns stdout', () => {
    const result = Bun.spawnSync([nodePath, '-e', `
      require(${polyfillRequire});
      const r = Bun.spawnSync([${JSON.stringify(nodePath)}, '-e', 'process.stdout.write("hello")'], { stdout: 'pipe' });
  ...
  test('Bun.spawn launches a process with pid', async () => {
    const result = Bun.spawnSync([nodePath, '-e', `
      require(${polyfillRequire});
      const p = Bun.spawn([${JSON.stringify(nodePath)}, '-e', 'process.stdout.write("test")'], { stdio: ['pipe', 'pipe', 'pipe'] });
