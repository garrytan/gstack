...
import { shouldEnableChromiumSandbox } from '../src/browser-manager';
...
describe('shouldEnableChromiumSandbox', () => {
  ...
  function policy(userNamespaceAvailable = true) {
    return shouldEnableChromiumSandbox(() => userNamespaceAvailable);
  }
  ...
  it('linux + unavailable user namespace → false (AppArmor fallback)', async () => {
    setPlatform('linux');
    process.getuid = (() => 1000) as typeof process.getuid;
    const { shouldEnableChromiumSandbox } = await import('../src/browser-manager');
    expect(shouldEnableChromiumSandbox(() => false)).toBe(false);
  });
});
