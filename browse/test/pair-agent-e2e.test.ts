...
describe('pair-agent flow end-to-end (HTTP only, no ngrok)', () => {
  ...
  test('POST /sse-session with root Bearer returns cookie and scoped SSE token', async () => {
  ...
  test('GET /activity/stream with scoped ?sse token is accepted', async () => {
    const minted = await fetch(`${daemon.baseUrl}/sse-session`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${daemon.token}` },
    });
    const body = await minted.json();
    await fetch(`${daemon.baseUrl}/command`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${daemon.token}`,
    ...
    const resp = await fetch(`${daemon.baseUrl}/activity/stream?sse=${encodeURIComponent(body.token)}`);
    expect(resp.status).toBe(200);
