...
describe('describeToolCall', () => {
  ...
  test('full browse binary path recognized', () => {
    const result = describeToolCall('Bash', { command: '/Users/garrytan/.antigravity/skills/gstack/browse/dist/browse goto https://example.com' });
    expect(result).toBe('Opening https://example.com');
  });
...
describe('per-tab agent concurrency', () => {
  ...
  test('spawnAntigravity accepts forTabId parameter', () => {
  ...
  test('sidebar-agent sends tabId with all events', () => {
    ...
    // askAntigravity should extract tabId from queue entry
  ...
  test('sidebar-agent allows concurrent agents across tabs', () => {
    ...
    // askAntigravity should be fire-and-forget (no await blocking the loop)
    expect(agentSrc).toContain('askAntigravity(entry).catch');
  });
...
describe('BROWSE_TAB tab pinning (cross-tab isolation)', () => {
  ...
  test('sidebar-agent passes BROWSE_TAB env var to antigravity process', () => {
