...
export interface PtySession {
   ...
   * spawnAgent's on-data callback emitted.
   */
  ringBuffer: Buffer[];
  ringBufferBytes: number;
  /**
   * Tracks whether the PTY is currently in xterm alt-screen mode. Agent
   * TUIs enter alt-screen (CSI ?1049h) during tool calls and exit (CSI
 ...
 * of dense agent output (including a recent tool result), small enough
 ...
 * multi-byte UTF-8 sequence or a partial ANSI CSI in half — the agent's
...
const AGENTS: Record<TerminalAgentName, TerminalAgentConfig> = {
  codex: {
    ...
    fallbackPaths: [
      ...
      path.join(process.env.HOME || '', '.bun/bin/codex'),
 ...
 * does not have to discover them by trial.
 */
...
function spawnAgent(agentName: TerminalAgentName, cols: number, rows: number, onData: (chunk: Buffer) => void) {
  ...
  // does not try to autostart its own. BROWSE_HEADED=1 keeps the existing
...
/**
 * Spawn the selected PTY for a session if it has not been spawned yet.
...
function maybeSpawnPty(ws: any, session: PtySession): boolean {
  ...
  if (!proc) {
    const selected = AGENTS[session.agent];
    try {
      ws.send(JSON.stringify({
        type: 'error',
        code: selected.missingCode,
        agent: selected.name,
        displayName: selected.displayName,
        installUrl: selected.installUrl,
        message: selected.displayName + ' not on PATH. Install: ' + selected.installUrl,
      }));
      ws.close(4404, selected.name + ' not found');
    } catch {}
    return false;
  }
  session.proc = proc;
...
function buildServer() {
  return Bun.serve({
    ...
    fetch(req, server) {
      ...
      // touching agent binary lookup (which can fail for non-process
      ...
      if (url.pathname === '/agent-available' && req.method === 'GET') {
        ...
        const selected = AGENTS[agentName];
        const found = findAgent(agentName);
        return new Response(JSON.stringify({
          agent: selected.name,
          displayName: selected.displayName,
      ...
      if (url.pathname === '/ws') {
        ...
        const upgraded = server.upgrade(req, {
          data: { cookie: token, sessionId, agent: agentName },
    ...
    websocket: {
      /**
       * Spawn the selected agent PTY for `session` if it hasn't been spawned yet.
      ...
      open(ws) {
        ...
        const agentName = (ws.data as any)?.agent || DEFAULT_TERMINAL_AGENT;

        // Commit 3 re-attach: if this sessionId already has a detached
        // PtySession in sessionsById, REPLACE its liveWs ref and replay
        // the ring buffer. The PTY process is unchanged — the agent keeps
        ...
        if (sessionId) {
          ...
          if (existing) {
            ...
            if (!existing.agent) existing.agent = agentName;
      ...
      message(ws, raw) {
        ...
        if (typeof raw === 'string') {
          ...
          if (msg?.type === 'start') {
            ...
            // immediately on every fresh WS so the selected agent boots without the
        ...
        // Binary input. Lazy-spawn the selected agent on the first byte if `start`
      ...
      close(ws, code, _reason) {
        ...
        // (code 4001 = restart, 4404 = no-agent error), dispose
        // immediately — there's no value in keeping the PTY alive.
        // Otherwise enter the detach window: the agent keeps running, the
