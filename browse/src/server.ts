...
const TUNNEL_COMMANDS = new Set<string>([
  ...
  'attrs', 'media', 'data',
  'scroll', 'press', 'type', 'select', 'wait', 'snapshot',
]);
...
async function start() {
  ...
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      //
      // Issues a short-lived view-only token. The local web UI can receive it
      // in an HttpOnly SameSite=Strict cookie; the Chrome extension also gets
      // the raw view-only token in the JSON body because EventSource cannot
      // send Authorization headers and SameSite cookies are not reliable from
      // chrome-extension:// origins. The token is valid ONLY on the SSE
      // endpoints (/activity/stream, /inspector/events); it is not a scoped
      // token and cannot be used against /command.
      //
      // The extension calls this once at bootstrap with the root Bearer
      // header, then opens EventSource with ?sse=<view-only-token>. This keeps
      // the root token out of URLs while preserving extension SSE behavior.
      ...
      if (url.pathname === '/activity/stream') {
        // Auth: Bearer header OR view-only SSE session token. EventSource
        // can't send Authorization headers. Browser pages can use the cookie;
        // extension pages use ?sse=<view-only-token>. The legacy ?token=<root>
        // query param is NO LONGER accepted — root URLs leak to logs/referer/
        // history. See N1 in the v1.6.0.0 security wave plan.
        const cookieToken = extractSseCookie(req);
        const sseToken = url.searchParams.get('sse');
        if (!validateAuth(req) && !validateSseSessionToken(cookieToken) && !validateSseSessionToken(sseToken)) {
          return new Response(JSON.stringify({ error: 'Unauthorized' }), {
            status: 401,
            headers: { 'Content-Type': 'application/json' },
          });
      ...
      if (url.pathname === '/inspector/events' && req.method === 'GET') {
        // Same auth model as /activity/stream: Bearer OR view-only SSE token.
