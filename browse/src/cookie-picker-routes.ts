...
const pendingCodes = new Map<string, { expiry: number, clearStorage: boolean }>();
...
const validSessions = new Map<string, { expiry: number, clearStorage: boolean }>();
const SESSION_TTL_MS = 3_600_000; // 1 hour

/** Generate a one-time code for opening the cookie picker UI. */
export function generatePickerCode(clearStorage: boolean = true): string {
  const code = crypto.randomUUID();
  pendingCodes.set(code, { expiry: Date.now() + CODE_TTL_MS, clearStorage });
  return code;
}
...
export function hasActivePicker(): boolean {
  ...
  for (const [code, val] of pendingCodes) {
    if (val.expiry > now) return true;
    pendingCodes.delete(code);
  }

  for (const [session, val] of validSessions) {
    if (val.expiry > now) return true;
...
function isValidSession(session: string): boolean {
  const val = validSessions.get(session);
  if (!val) return false;
  if (Date.now() > val.expiry) { validSessions.delete(session); return false; }
  return true;
}
...
export async function handleCookiePickerRoute(
  ...
  try {
    ...
    if (pathname === '/cookie-picker' && req.method === 'GET') {
      ...
      if (code) {
        const val = pendingCodes.get(code);
        ...
        validSessions.set(session, { expiry: Date.now() + SESSION_TTL_MS, clearStorage: val.clearStorage });
        return new Response(null, {
          status: 302,
          headers: {
    ...
    if (pathname === '/cookie-picker/import' && req.method === 'POST') {
      ...
      let keepStorage = body.keepStorage;
      if (keepStorage === undefined) {
        const sessionCookie = getSessionFromCookie(req);
        if (sessionCookie && validSessions.has(sessionCookie)) {
          keepStorage = !validSessions.get(sessionCookie).clearStorage;
        }
      }
