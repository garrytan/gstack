...
// Opening a native browser window can take longer than a network request,
// especially when macOS is waking Chrome. Keep the code one-time, but give
// the hand-off enough time to complete.
  ...
  try {
    ...
    if (pathname === '/cookie-picker' && req.method === 'GET') {
      ...
      if (code) {
        ...
        const location = targetDomain
          ? `/cookie-picker?targetDomain=${encodeURIComponent(targetDomain)}`
          : '/cookie-picker';
        return new Response(null, {
          status: 302,
          headers: {
      ...
      if (session && isValidSession(session)) {
        const html = getCookiePickerHTML(port, url.searchParams.get('targetDomain'));
    ...
    if (pathname === '/cookie-picker/profiles' && req.method === 'GET') {
      ...
      let recommendedProfile: string | undefined;
      if (targetDomain) {
