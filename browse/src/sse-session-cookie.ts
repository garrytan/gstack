/**
 * View-only session token registry for SSE endpoints.
 *
 ...
 * a separate short-lived token, scoped to SSE reads only. Browser pages can
 * receive it in an HttpOnly SameSite=Strict cookie; Chrome-extension pages
 * use scoped `?sse=<VIEW_ONLY>` because SameSite cookies are not reliable
