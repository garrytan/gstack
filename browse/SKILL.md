...
| `record start [output.mp4] | record mark <label> | record status | record stop` | Record the active browser tab to an H.264 MP4 while preserving the current gstack session. Interactions receive safe on-video markers and a JSON timeline is saved beside the video. |
