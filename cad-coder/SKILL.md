...
3. **Session state is durable.** Every turn reads `cad-out/<part>.session.json`,
...
Write the script to `cad-out/<part-name>.py` and the session file to
`cad-out/<part-name>.session.json`. Create `cad-out/` if missing; add it
...
`cad-out/<part-name>.session.json` — read-mutate-write every turn.

```json
{
  "part": "mounting-bracket",
  "script": "cad-out/mounting-bracket.py",
...
{
  "part": "motor-bracket",
  "script": "cad-out/motor-bracket.py",
  "created_at": "2026-05-16T11:50:00Z",
  "mode": "engineered",
...
```
Built: cad-out/mounting-bracket.step  (turn 1)
...
```
Built: cad-out/motor-bracket.step  (turn 1, engineered)
...
| "save this as a variant" / "branch" | **Snapshot** | `cp cad-out/<part>.{py,session.json} cad-out/<part>-<tag>.{py,session.json}`. |
...
3. **Don't commit `cad-out/`.** Add to `.gitignore` if not already
   there. The `.py` is the source; STEP/STL/session are artifacts.
...
5. **One part per session.** If the user describes a second part,
   ...
   reference siblings in the report ("see also `cad-out/lid.session.json`").
