...
**Design doc check:**
If you are in plan mode, first check the conversation context for an active plan file path
(Claude Code usually stores it under `~/.claude/plans/`, Codex under `~/.codex/plans/`).
Read the first 30 lines of that active plan file. If it looks like an `/office-hours`
design doc, for example it contains `Generated by /office-hours`, `# Design:`, `Mode: Startup`,
or `Mode: Builder`, treat it as `PLAN_DESIGN`.

Then check the persisted project-scoped design doc:
```bash
SLUG=$(~/.claude/skills/gstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.gstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.gstack/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
[ -n "$DESIGN" ] && echo "Persisted design doc found: $DESIGN" || echo "No persisted design doc found"
```
If both `PLAN_DESIGN` and `$DESIGN` exist, prefer whichever is newer. If timestamps are
unclear, prefer `PLAN_DESIGN`, because plan-mode edits continue there after `/office-hours`
while the `.gstack` copy is only the last persisted snapshot. Use the chosen design doc as
the source of truth for the problem statement, constraints, and chosen approach. If it has
a `Supersedes:` field, note that this is a revised design.
...
If you are in plan mode, also check the conversation context for the active plan file path
(usually under `~/.claude/plans/` or `~/.codex/plans/`). Read that file first. If it
looks like an `/office-hours` design doc (`Generated by /office-hours`, `# Design:`,
`Mode: Startup`, or `Mode: Builder`), treat it as `PLAN_DESIGN`.

If both `PLAN_DESIGN` and `$DESIGN` exist, prefer whichever is newer. If timestamps are
unclear, prefer `PLAN_DESIGN`, because plan-mode edits continue there after `/office-hours`
while the `.gstack` copy is only the last persisted snapshot.

If a design doc is now found, read the chosen one and continue the review.
