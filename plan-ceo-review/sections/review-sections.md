...
trace an adversarial interleaving with another operation before resuming. Which
assumptions still hold? A single-threaded event loop and atomic individual calls
do not make the whole flow atomic. For an ordering guarantee, specify a test
that pauses and releases operations at those boundaries, not only sequential calls.
