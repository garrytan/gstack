...
**Dedicated tools over Bash.** Where the harness has not directed otherwise, prefer
Read, Edit, Write, Glob, Grep over shell equivalents (cat, sed, find, grep).
...
Unrelated work (rewrites, multi-quarter migrations) is separate scope — flag it rather than folding it in.
...
Read `GSTACK_PLAN_MODE` from the environment (emitted by `## Preamble (run first)`'s preamble bash). Then:
