...
const searchSchema = z.object({
  ...
  persona: z.enum(BUYER_PERSONAS).default('homebuyer'),
...
export default async function SearchPage({
  ...
  let scored = await buildSummaries(result.items, { persona: params.persona as BuyerPersona });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      <TrackView
        ...
        properties={{ query: params.q ?? '', results: result.total, persona: params.persona }}
      />
      ...
      <p className="mt-1 text-sm text-[var(--text-secondary)]">
        Results are scored for a <strong>{params.persona}</strong>. Change the persona and the
        weighting changes with it.
      </p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[240px_1fr]">
        <aside>
          <SearchFilters localities={localities} current={params} />
        </aside>
