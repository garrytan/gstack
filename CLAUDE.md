...
**Before editing browser/sidebar/server internals, read the relevant ARCHITECTURE.md
section first — these are load-bearing and CI tripwires enforce several of them:**

- **Sidebar, PTY & embedder lifecycle** (WebSocket `Sec-WebSocket-Protocol` auth,
  `ownsTerminalAgent` teardown, cross-pane `gstackInjectToTerminal`, the `/health`
  token rule) — before editing `sidepanel.js`, `background.js`, `content.js`,
  `terminal-agent.ts`, `server.ts`, or sidebar endpoints:
  [ARCHITECTURE.md](ARCHITECTURE.md#sidebar--pty-lifecycle) +
  [docs/designs/SIDEBAR_MESSAGE_FLOW.md](docs/designs/SIDEBAR_MESSAGE_FLOW.md).
- **Dual-listener tunnel security** (`pair-agent` local vs tunnel listener, scoped
  tokens, SSE session cookies) — before editing `server.ts`, `sse-session-cookie.ts`,
  or `tunnel-denial-log.ts`:
  [ARCHITECTURE.md](ARCHITECTURE.md#dual-listener-tunnel-architecture-v1600). The module
  boundary (no imports from `token-registry.ts` into `sse-session-cookie.ts`) is
  load-bearing for scope isolation.
- **Unicode sanitization at server egress** — before adding any SSE/WebSocket writer or
  HTTP response in `server.ts` that ships page-derived strings:
  [ARCHITECTURE.md](ARCHITECTURE.md#unicode-sanitization-at-server-egress-v13800).
  Bypasses fail `browse/test/server-sanitize-surrogates.test.ts`.
- **Sidebar prompt-injection defense** (layer stack, thresholds, `GSTACK_SECURITY_*`
  env knobs) — before touching `content-security.ts`, `security-classifier.ts`, or
  `security.ts`: [ARCHITECTURE.md](ARCHITECTURE.md#prompt-injection-defense-sidebar-agent).
  Critical: `security-classifier.ts` CANNOT be imported from the compiled browse binary
  (`@huggingface/transformers` needs `onnxruntime-node`, which fails `dlopen` from Bun's
  temp extract dir) — only the pure-string `security.ts` is safe for `server.ts`.

**Setup symlink hardening** (v1.38.0.0+). Every link site in `setup` MUST route through
the `_link_or_copy SRC DST` helper near the `IS_WINDOWS` detection. On Windows without
Developer Mode, plain `ln -snf` produces frozen file copies that don't refresh on
`git pull`. `test/setup-windows-fallback.test.ts` fails CI on a single raw `ln` call
...
**Don't chase the number.** Fix patterns that are real quality problems; accept findings
where the "sloppy" pattern is the correct engineering choice. The full judgment-call guide
(which `catch {}` to fix vs leave, extension crash semantics, best-effort cleanup) and the
score history live in **[docs/SLOP_SCAN.md](docs/SLOP_SCAN.md)**.
...
When the user says "check out <PR link>" and the PR is from a fork that is NOT a
collaborator on `garrytan/gstack` (e.g. `garrytan-agents/gstack`), don't just
`gh pr checkout` — fork PRs don't receive base-repo secrets, so eval/E2E CI fails with
empty-env auth. Push the branch to the base repo and re-target the PR. Full steps:
**[CONTRIBUTING.md](CONTRIBUTING.md#checking-out-fork-prs-secret-access)**.

## CHANGELOG + VERSION style

Full style + versioning bible: **[docs/CHANGELOG_STYLE.md](docs/CHANGELOG_STYLE.md)**.
The operational ship-time checklist (enumerate commits → group → write → cross-check)
lives in the `/ship` skill. Read the full doc before writing or restructuring a
CHANGELOG entry. The invariants in brief:

- **VERSION is a monotonic ordered release id, not a strict semver contract.** The
  bump level (major/minor/patch/micro) signals intent at ship time. Scale the bump to
  the diff: big diff → MINOR/MAJOR, never PATCH. Don't undersell.
- **VERSION and CHANGELOG are branch-scoped.** Every shipping branch gets its own bump
  + entry on top, even after merging main. Never edit an entry already on main; merging
  main does NOT mean adopting main's version.
- **The entry is the diff between main and your branch — what users get on upgrade.**
  Never narrate branch history (internal version bumps, mid-branch fixes, reviews,
  scope negotiations). Lead with what users can now do, in plain language. No TODOS.md,
  no internal tracking.
- **Every `## [X.Y.Z]` entry opens with a release-summary** (two-line headline → lead →
  "the numbers that matter" table → "what this means" close) in the GStack/Garry voice,
  then `### Itemized changes` below. No em dashes, no AI vocabulary, real numbers only.
  Credit community PRs with `Contributed by @username`.
- After any entry move/add/remove, run `grep "^## \[" CHANGELOG.md` to verify ordering.
  Gaps between version numbers are fine; never back-fill placeholders.
...
Native OpenClaw skills (`openclaw/skills/gstack-openclaw-*`) publish to ClawHub via
`clawhub publish` (NOT `clawhub skill publish`). Per-skill commands, auth, and
verification: **[docs/OPENCLAW.md](docs/OPENCLAW.md#publishing-to-clawhub)**.
