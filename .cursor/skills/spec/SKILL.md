...
- **1+ matches:** surface them to the user via AskQuestion: "Found {N} similar
...
AskQuestion: A) edit, B) acknowledge and proceed, C) cancel. **On a PUBLIC repo,
...
block; MEDIUM PII/legal/internal confirm via AskQuestion; LOW surfaces). Full
...
2. **Exit 2 (MEDIUM)** — AskQuestion per finding (cluster identical ids; PUBLIC
...
- **Score <7, iteration 2:** print "Quality gate: {score}/10 (after one
  revision). Codex still flags: {ambiguities}." AskQuestion:
  - A) Ship anyway (file at this quality)
  - B) Save draft locally and stop (no issue filed)
  - C) One more revision attempt

Max 3 dispatches total. If still <7 after iter 3, AskQuestion same options.
...
If `$DIRTY` is non-empty, AskQuestion:
...
from the answer, re-prompt the AskQuestion. The check must happen INSIDE
...
**D16 mandatory final-confirm gate:** AskQuestion: "Spawn agent now? Last
...
- `T_PHASE1_START` — Phase 1 first AskQuestion or first text emit
...
`AskQuestion`. For open-ended interrogation, ask inline in the chat — the
