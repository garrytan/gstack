...
---
<!-- Generated by scripts/gen-cursor-native.ts from design-review/SKILL.md.tmpl — do not edit directly -->
...
If the output is non-empty (working tree is dirty), **STOP** and use AskQuestion:
...
**If NO ecosystem marker matched:** Use AskQuestion:
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. AskQuestion: "This site requires authentication. Want to import cookies from your browser? Run `/setup-browser-cookies` first if needed."
...
11. **Clean working tree required.** If dirty, use AskQuestion to offer commit/stash/abort before proceeding.
