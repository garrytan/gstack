...
---
<!-- Generated by scripts/gen-cursor-native.ts from plan-design-review/SKILL.md.tmpl — do not edit directly -->
...
|------|-------------------|
| running the 7 design passes, required outputs, and review report (only after Step 0 scope is agreed) | `$GSTACK_ROOT/plan-design-review/sections/review-sections.md` |
---
...
### 0D. Focus Areas
AskQuestion: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll generate visual mockups next, then review all 7 dimensions. Want me to focus on specific areas instead of all 7?"
...
path; use this for the AskQuestion URL AND as the base for the reload
...
**PRIMARY WAIT: AskQuestion with board URL**

After the board is serving, use AskQuestion to wait for the user. Include the
...
**Do NOT use AskQuestion to ask which variant the user prefers.** The comparison
board IS the chooser. AskQuestion is just the blocking wait mechanism.

**After the user responds to AskQuestion:**
...
5. Reload the board in the user's browser (same tab) — the URL is per-board
   ...
   daemon.
6. The board auto-refreshes. **AskQuestion again** with the same board URL to
   wait for the next round of feedback. Repeat until `feedback.json` appears.

**If `NO_FEEDBACK_FILE`:** The user typed their preferences directly in the
AskQuestion response instead of using the board. Use their text response
as the feedback.
...
Use AskQuestion to verify before proceeding.
...
**Do NOT use AskQuestion to ask which variant the user picked.** Read `feedback.json` — it already contains their preferred variant, ratings, comments, and overall feedback. Only use AskQuestion to confirm you understood the feedback correctly, never to re-ask what they chose.
...
5. AskQuestion if there's a genuine design choice to resolve
...
> **STOP.** Before running the 7 design passes, required outputs, and review report (only after Step 0 scope is agreed), Read `plan-design-review/sections/review-sections.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

## Section self-check (before you finish)

Confirm you Read the review section the Section index named, and executed all 7 design passes, the required outputs, and the review report in full. If you produced findings or the review report from memory without Reading `$GSTACK_ROOT/plan-design-review/sections/review-sections.md`, stop and Read it now.
