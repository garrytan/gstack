...
---
<!-- Generated by scripts/gen-cursor-native.ts from setup-gbrain/SKILL.md.tmpl — do not edit directly -->
...
dead Postgres URL). Fire a targeted AskQuestion BEFORE Step 2:
...
`broken-config`, fire the same AskQuestion again (the user picks again).
...
- `"offer": true` → present the returned options ONCE via AskQuestion:
...
## Step 2: Pick a path (AskQuestion)
...
Do NOT silently pick; fire the AskQuestion.
...
Ask the D17 tier prompt via AskQuestion: "Which Supabase tier?" Present
...
List orgs, pick one (AskQuestion if multiple):
...
- `deny` → do nothing.
- `unset` → AskQuestion: "How should `<normalized-remote>` interact with
  gbrain?"
...
Separate AskQuestion: "Also sync your gstack artifacts (CEO plans,
...
Otherwise (the "many transcripts on disk" path): AskQuestion with
...
(local engines are inherently single-tenant). No AskQuestion.
...
trust policy question via AskQuestion:
...
For each orphan, AskQuestion per project: "Delete orphan project
