...
---
<!-- Generated by scripts/gen-cursor-native.ts from investigate/SKILL.md.tmpl — do not edit directly -->
...
1. **Collect symptoms:** Read the error messages, stack traces, and reproduction steps. If the user hasn't provided enough context, ask ONE question at a time via AskQuestion.
...
_FREEZE_SCRIPT="$HOME/$GSTACK_ROOT/freeze/bin/check-freeze.sh"
...
3. **3-strike rule:** If 3 hypotheses fail, **STOP**. Use AskQuestion:
   ```
...
5. **If the fix touches >5 files:** Use AskQuestion to flag the blast radius:
   ```
...
- **If fix touches >5 files → AskQuestion** about blast radius before proceeding.
- **Completion status:**
