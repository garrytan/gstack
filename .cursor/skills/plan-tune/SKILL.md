...
---
<!-- Generated by scripts/gen-cursor-native.ts from plan-tune/SKILL.md.tmpl — do not edit directly -->
...
2. AskQuestion (use the contributor-specific framing only if `_CONTRIB=true`,
   ...
   **Contributor framing (only if `_CONTRIB=true`):**
   ...
   > AskQuestion outcome locally to
...
1. Ask FIVE one-per-dimension declaration questions via individual
   AskQuestion calls (one at a time). Use plain English, no jargon:
...
2. For each unapplied proposal, present it as a numbered item and use
   AskQuestion (one per call, per skill convention). Show:
...
3. If `NO_FREE_TEXT`: tell the user "No free-text answers since the last
   distill. Keep using gstack — `Other` responses on AskQuestion feed
   this loop."
