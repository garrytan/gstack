...
---
<!-- Generated by scripts/gen-cursor-native.ts from sync-gbrain/SKILL.md.tmpl — do not edit directly -->
...
If `_POLICY == "unset"` AND `_HASH != "local"`, AskQuestion per the
...
not `--full`, AskQuestion via the format in the preamble:
...
`PAGES > 0`, AskQuestion via the format in the preamble:
