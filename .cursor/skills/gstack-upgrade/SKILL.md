...
---
<!-- Generated by scripts/gen-cursor-native.ts from gstack-upgrade/SKILL.md.tmpl — do not edit directly -->
...
**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=1`:** Skip AskQuestion. Log "Auto-upgrading gstack v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/gstack-upgrade` manually to retry."
...
```bash
if [ -d "$HOME/$GSTACK_ROOT/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/$GSTACK_ROOT"
