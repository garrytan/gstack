...
---
<!-- Generated by scripts/gen-cursor-native.ts from ship/SKILL.md.tmpl — do not edit directly -->
...
A step sometimes requires action on an external website the user controls: registering an API key, creating a vendor or developer account, configuring a dashboard, webhook, OAuth app, billing plan, or domain verification. This contract governs that moment. It grants no new browsing authority — the AskQuestion format and one-way-door rules remain binding, including approval before anything that spends money.
...
|------|-------------------|
| the ship target is an Apple platform app (.xcodeproj, .xcworkspace, or an app-product Swift package) — read BEFORE Step 1's branch gate and any preflight; store distribution never routes through the branch/PR ceremony | `$GSTACK_ROOT/ship/sections/apple-release.md` |
| running the test suites and (if prompt files changed) the eval suites (Steps 4-6) | `$GSTACK_ROOT/ship/sections/tests.md` |
| auditing test coverage of the diff (Step 7) | `$GSTACK_ROOT/ship/sections/test-coverage.md` |
| auditing plan completion, verification, and scope drift (Step 8) | `$GSTACK_ROOT/ship/sections/plan-completion.md` |
| the pre-landing review and specialist dispatch (Step 9) | `$GSTACK_ROOT/ship/sections/review-army.md` |
| addressing Greptile review comments when a PR exists (Step 10) | `$GSTACK_ROOT/ship/sections/greptile.md` |
| the adversarial review and learnings capture (Step 11) | `$GSTACK_ROOT/ship/sections/adversarial.md` |
| writing the CHANGELOG entry (Step 13) | `$GSTACK_ROOT/ship/sections/changelog.md` |
| syncing docs and creating or updating the PR/MR (Steps 18-19) | `$GSTACK_ROOT/ship/sections/pr-body.md` |
...
3. **If no release pipeline exists and a new artifact was added:** Use AskQuestion:
...
> **STOP.** Before running the test suites and (if prompt files changed) the eval suites (Steps 4-6), Read `ship/sections/tests.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing test coverage of the diff (Step 7), Read `ship/sections/test-coverage.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing plan completion, verification, and scope drift (Step 8), Read `ship/sections/plan-completion.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the pre-landing review and specialist dispatch (Step 9), Read `ship/sections/review-army.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before addressing Greptile review comments when a PR exists (Step 10), Read `ship/sections/greptile.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the adversarial review and learnings capture (Step 11), Read `ship/sections/adversarial.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
1. **Classify state** — pure reader, never writes:
   ...
   - **ALREADY_BUMPED** → skip the bump, but run the queue-drift check (step 3) with the reported `currentVersion`. If the queue moved (next free version differs), **AskQuestion**: rebump to the new version (rewrites CHANGELOG header + PR title) or keep current (CI version-gate will reject until resolved).
...
3. **Queue-aware pick** (workspace-aware ship):
   ...
   ```
   If `offline`/util fails: fall back to local `BUMP_LEVEL` arithmetic and print `⚠ workspace-aware ship offline — using local bump only`. If `claimed` is non-empty, render the queue table so the user sees landing order. If an active sibling workspace holds a version `>= NEW_VERSION`, **AskQuestion**: advance past (unrelated work) or abort and sync with the sibling.
...
> **STOP.** Before writing the CHANGELOG entry (Step 13), Read `ship/sections/changelog.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
**If TODOS.md does not exist:** Use AskQuestion:
...
**If disorganized** (missing priority fields, no component groupings, no Completed section): Use AskQuestion:
...
user via AskQuestion rather than destroying non-WIP commits.
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   offer (fires once EVER, machine-wide). AskQuestion:
   ...
   render — a failed AskQuestion must re-offer next time):
   ```bash
...
> **STOP.** Before syncing docs and creating or updating the PR/MR (Steps 18-19), Read `ship/sections/pr-body.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
if [ ! -f "$_NUDGE_MARKER" ] && [ "$_QT" = "false" ]; then
  echo ""
  echo "gstack can learn from your AskQuestion answers. Run /plan-tune to opt in"
