...
---
<!-- Generated by scripts/gen-cursor-native.ts from office-hours/SKILL.md.tmpl — do not edit directly -->
...
|------|-------------------|
| writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done) | `$GSTACK_ROOT/office-hours/sections/design-and-handoff.md` |
---
...
Ask these questions **ONE AT A TIME** via AskQuestion. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** via AskQuestion. The goal is to brainstorm and sharpen the idea, not interrogate.
...
- Ask via AskQuestion: "Should we build on this prior design or start fresh?"
...
**Privacy gate:** Before searching, use AskQuestion: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
...
Use AskQuestion to confirm. If the user disagrees with a premise, revise understanding and loop back.
...
Use AskQuestion (regardless of codex availability):
...
6. **Premise revision check:** If Codex challenged an agreed premise, use AskQuestion:
...
Emit ONE AskQuestion that lists every alternative (A/B and optionally C) as numbered options, using the preamble's AskQuestion Format section. The AskQuestion call is a tool_use, not prose — write the question text and call the tool.
...
If `$D serve` is not available or fails, fall back to AskQuestion:
...
If Codex is available, use AskQuestion:
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `office-hours/sections/design-and-handoff.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

## Section self-check (before you finish)

Confirm you Read every section the Section index named as applying to this run, and executed it in full. The design doc and the handoff are the deliverables — if you produced them from memory without Reading `$GSTACK_ROOT/office-hours/sections/design-and-handoff.md`, stop and Read it now.
...
- **Questions ONE AT A TIME.** Never batch multiple questions into one AskQuestion.
