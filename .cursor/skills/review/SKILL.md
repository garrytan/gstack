...
---
<!-- Generated by scripts/gen-cursor-native.ts from review/SKILL.md.tmpl — do not edit directly -->
...
- **HIGH-impact discrepancies** trigger AskQuestion:
  - Show the investigation findings
  - Options: A) Stop and implement missing items, B) Ship anyway + create P1 TODOs, C) Intentionally dropped

This is **INFORMATIONAL** unless HIGH-impact discrepancies are found (then it gates via AskQuestion).
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `review/design-checklist.md`
...
1. The red-team checklist from `review/specialists/red-team.md`
...
If there are ASK items remaining, present them in ONE AskQuestion:
...
If 3 or fewer ASK items, you may use individual AskQuestion calls instead of batching.
...
2. **FALSE POSITIVE comments:** Present each one via AskQuestion:
...
3. **VALID BUT ALREADY FIXED comments:** Reply using the **Already Fixed reply template** from greptile-triage.md — no AskQuestion needed:
...
If GATE is FAIL, use AskQuestion:
```
