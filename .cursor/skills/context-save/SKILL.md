...
---
<!-- Generated by scripts/gen-cursor-native.ts from context-save/SKILL.md.tmpl — do not edit directly -->
...
- **Infer, don't interrogate.** Use git state and conversation context to fill in
  the file. Only use AskQuestion if the title genuinely cannot be inferred.
- **This is a gstack skill, not a Claude Code built-in.** When the user types
  `/context-save`, invoke this skill by reading `.cursor/skills/<name>/SKILL.md`. The old `/checkpoint`
