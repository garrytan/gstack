...
---
<!-- Generated by scripts/gen-cursor-native.ts from land-and-deploy/SKILL.md.tmpl — do not edit directly -->
...
to guide deploy verification. If nothing is detected, ask the user via AskQuestion
...
Present the full dry-run results to the user via AskQuestion:
...
```bash
$GSTACK_ROOT/bin/gstack-evidence check --label tests --expect-cmd '<the project test command>' --max-age 24 --allow-paths CHANGELOG.md,VERSION,package.json
```
...
4. If no deploy workflows detected and no URL provided: use AskQuestion once:
...
If deploy fails (`conclusion` is `failure`): use AskQuestion:
...
If any fail: show the evidence (screenshot path, console errors, perf numbers). Use AskQuestion:
