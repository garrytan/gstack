...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.copilot/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
## Phase 2A: Startup Mode — Strategic Product Diagnostic
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.copilot/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.copilot/projects/`.
...
- If the second opinion reviewer proposed a prototype in Phase 3.5, consider using it as a starting point for the creative/lateral approach.
...
Before writing the design doc, synthesise the signals you observed during the session. These will appear in the design doc ("What I noticed") and inform the Beat 1 reflection in the closing.
...
- **Defended premise with reasoning** against second-opinion challenge (kept the original premise and articulated specific reasoning for why — dismissal without reasoning does not count)

Count the signals. You'll reference them in the Beat 1 reflection — the more specific signals you can name, the stronger the closing.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t ~/.copilot/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.copilot/projects/{slug}/{user}-{branch}-design-{datetime}.md`:
...
{If a second opinion ran in Phase 3.5: independent cold read — steelman, key insight, challenged premise, prototype suggestion. Verbatim or close paraphrase. If a second opinion did NOT run (skipped or unavailable): omit this section entirely — do not include it.}
...
## Cross-Model Perspective
{If a second opinion ran in Phase 3.5: independent cold read — coolest version, key insight, existing tools, prototype suggestion. Verbatim or close paraphrase. If a second opinion did NOT run (skipped or unavailable): omit this section entirely — do not include it.}
...
## Phase 6: Session Close — Reflection & Next Steps

Once the design doc is APPROVED, deliver a clean closing. Two beats.

### Beat 1: Signal Reflection

One paragraph reflecting the quality of thinking demonstrated in the session.
Reference actual things the user said — quote their words back. Be specific, not generic.

**Anti-slop rule — show, don't tell:**
- GOOD: "You didn't say 'small businesses' — you said 'the ops manager at a regional
  logistics firm who reconciles 400 transactions manually every Friday.' That level
  of specificity is what separates a real problem from a hypothesis."
- BAD: "You showed great specificity in identifying your target user."
- GOOD: "You pushed back on premise #2 and had a reason. That's not stubbornness —
  that's product conviction."
- BAD: "You demonstrated conviction and independent thinking."

Keep it to 3-4 sentences. Name specific moments. This is a mirror, not a pep talk.

### Beat 2: What Comes Next

Surface the natural next step in the Global Payments product development journey.
Base this on what the design doc contains and what stage the work is at.

**If the idea is pre-investment / pre-roadmap:**
> "The design doc is your starting point. The next step is pressure-testing the
> business case — does this generate enough value to warrant the investment?
> Run `/business-case` to build that out, then `/plan-ceo-review` to challenge
> the scope before it gets locked."

**If the idea is scoped and ready for planning:**
> "You've got a solid design. Time to stress-test the architecture and map the
> delivery plan. Run `/plan-eng-review` to lock down the technical approach,
> then `/roadmap-plan` to sequence it against real capacity."

**If the idea touches compliance (payments, data, regulated flows):**
> "Before this goes any further, route it through compliance. Run `/privacy`
> if personal data is involved, `/pci-review` if card data is in scope, and
> `/security-threat-model` for any new API surface or trust boundary.
> These are faster to run now than to retrofit after build."

**If this is preparing for a governance review or QBR:**
> "Take the design doc straight into `/governance` — it will structure the
> artefacts you need for your checkpoint review and flag any gaps before
> you're in the room."

**If the mode was Builder / internal project:**
> "Design doc is saved. Next is `/plan-eng-review` to lock the architecture,
> then `/launch-readiness` when you're close to go-live. If you want to run
> all four plan reviews in one pass, use `/autoplan`."

Always close with:
- The specific command to type next
- One line on what that skill will produce and why it matters now

**Do not offer external resources, links, or anything outside the GP skill
ecosystem. The journey continues inside the tools.**

### Next-skill routing (reference)

After the close, suggest the most relevant next step:

- `/plan-ceo-review`       — ambitious scope, needs premise challenge
- `/business-case`         — needs investment justification
- `/plan-eng-review`       — ready to lock architecture
- `/plan-design-review`    — needs UX/design review before build
- `/privacy`               — touches personal data
- `/pci-review`            — touches card data
- `/security-threat-model` — new APIs, integrations, or trust boundaries
- `/governance`            — approaching a checkpoint or QBR
- `/roadmap-plan`          — needs sequenced delivery plan
- `/autoplan`              — run all four plan reviews in one pass

The design doc at `~/.copilot/projects/` is automatically discoverable by
downstream skills — they will read it during their context load.
