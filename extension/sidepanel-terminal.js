...
(function () {
   ...
   * receive an unrecoverable signal (auth invalid → 401, selected agent
   * not found, fatal server error) so the loop doesn't keep retrying and
  ...
  function openReattachWebSocket(terminalPort, attachToken, sessionId) {
    ...
    ws = new WebSocket('ws://127.0.0.1:' + terminalPort + '/ws?agent=' + encodeURIComponent(currentAgent), [`gstack-pty.${attachToken}`]);
  ...
  async function checkAgentAvailable(terminalPort, agentKey = currentAgent) {
    try {
   ...
   * action so the user can drive the selected agent from outside-the-keyboard surfaces.
  ...
  async function connect() {
    ...
    // Pre-flight: does the selected agent exist on PATH?
    const agentStatus = await checkAgentAvailable(terminalPort);
    ...
    ws.addEventListener('open', () => {
      ...
      // v1.44 eager spawn: send {type:"start"} so the agent boots
    ...
    ws.addEventListener('close', (ev) => {
      ...
      if (state === STATE.NO_AGENT) return;
      // v1.44 Commit 3 — re-attach loop on transient close. Clean codes
      // (1000 = pty exited, 4001 = intentional restart, 4404 = no-agent)
  ...
  async function forceRestart() {
    ...
    setState(STATE.IDLE, { message: 'Restarting ' + getAgent().displayName + '...' });
    ...
    // Pre-flight: selected agent still on PATH?
    const agentStatus = await checkAgentAvailable(terminalPort);
    if (!agentStatus.available) {
      setState(STATE.NO_AGENT);
      return;
    ...
    setState(STATE.LIVE);
    ensureXterm();
    ws = new WebSocket('ws://127.0.0.1:' + terminalPort + '/ws?agent=' + encodeURIComponent(currentAgent), [`gstack-pty.${token}`]);
    ws.binaryType = 'arraybuffer';
    ...
    ws.addEventListener('open', () => {
      ...
      // Eager spawn — fresh agent prompt visible without user keystroke.
      try { ws.send(JSON.stringify({ type: 'start' })); } catch {}
      if (keepaliveInterval) clearInterval(keepaliveInterval);
      keepaliveInterval = setInterval(() => {
        if (!ws || ws.readyState !== WebSocket.OPEN) return;
