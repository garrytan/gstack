...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'nstack-status-pill';
  ...
  statusPill.innerHTML = `<span class="nstack-pill-dot"></span> nstack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'nstack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'nstack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'nstack-ref-panel';

  const header = document.createElement('div');
  header.className = 'nstack-ref-panel-header';
  header.textContent = `nstack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'nstack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'nstack-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'nstack-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'nstack-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'nstack-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'nstack-ref-panel-more';
...
// Do NOT dispatch nstack-extension-ready here — the extension being loaded
