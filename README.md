...
| `/sync-gbrain` | **Keep Brain Current** — sync this repo's changed Markdown and code into gbrain via `gbrain sources add` + `gbrain sync --strategy auto`, refresh the `## GBrain Search Guidance` block in CLAUDE.md, and auto-remove guidance when the capability check fails. Repository images remain excluded until GBrain preserves source isolation for image imports. `--incremental` (default), `--full`, `--dry-run`. Idempotent; safe to re-run. |
...
**Keeping the brain current.** Run `/sync-gbrain` from any repo to sync its repository contents into gbrain (incremental by default, `--full` for the auto walk followed by a full code reindex, `--dry-run` to preview). The skill registers the cwd as a federated source via `gbrain sources add`, then runs `gbrain sync --strategy auto`: GBrain admits eligible Markdown and code when they are new or changed. Repository images are deliberately disabled on this path until GBrain preserves the selected source for image imports. On a source with a usable anchor, switching to this strategy alone does not revisit unchanged historical Markdown. GBrain may still run an authoritative full reconciliation on first sync, when its anchor or history is unavailable, or when its indexing pipeline changes. The second `--full` phase is `gbrain reindex-code`. The skill also writes a `## GBrain Search Guidance` block to your project's CLAUDE.md so the agent prefers `gbrain search`/`code-def`/`code-refs` over Grep. The block is removed automatically if the capability check fails — no stale guidance pointing at tools that aren't installed.
