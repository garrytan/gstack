...
`--host auto` installs every supported host it can actually detect. When Claude and/or Codex are present, it materializes shared runtime assets in `~/.gstack` and host-specific discoverable entrypoints in locations like `~/.claude/skills/gstack` and `~/.codex/skills/gstack`. If only Gemini is detected, run it from the target repo root so setup can materialize `.gstack`; otherwise setup now fails instead of silently defaulting to Claude. Hook-based safety skills (careful, freeze, guard) use inline safety advisory prose on non-Claude hosts.
