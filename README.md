...
| `/gloop` | **Delivery Lead** | Drive a goal to PR-ready through the whole loop: clarify into verifiable success criteria, review the plan with `/autoplan` or `/plan-eng-review` before any code, implement in scoped passes, run your tests and `/review` after each pass, loop until done or blocked. The opposite of a raw goal-to-code loop — independent review every pass, major scope changes go to you. Hands off to `/ship`. |
