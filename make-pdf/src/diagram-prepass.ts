 ...
 *
 * Bundle calls are batched: Aside runs one script per `aside repl` process and
...
import { renderTmpDir, renderWithAside } from "../../lib/aside-render";
...
// ─── Bundle runner (diagram-render page, driven through Aside) ────────
...
/** Aside caps a script at 120s; ~40 mermaid renders fit with room to spare. */
 ...
 */
export function bundleRunner(opts: { bundlePath?: string; render?: typeof renderWithAside } = {}): BundleRun {
  const render = opts.render ?? renderWithAside;
...
/**
 * One Aside script: open the bundle, wait for #done, evaluate one expression
