 ...
 *   exit 0 success / 1 bad args / 2 render error / 3 Paged.js timeout / 4 Aside unavailable.
 */

import { COMMANDS } from "./commands";
import { ExitCode, AsideClientError } from "./types";
...
function printUsage(): void {
  ...
  lines.push("");
  lines.push("Run `$P setup` to verify the Aside browser + pdftotext install.");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (err instanceof AsideClientError) {
      console.error(`$P: ${err.message}`);
      process.exit(ExitCode.AsideUnavailable);
    }
    if (err?.code === "ENOENT") {
