 ...
 *
 * Every browser step is its own `aside repl` script (nothing persists between
 * them): one batch for diagram fences, one for oversized-image downscales,
 * one for DOCX rasters, one print. Parallel $P generate calls never share
 * state.
 */
...
export async function generate(opts: GenerateOptions): Promise<string> {
  ...
  // missing bundle or closed Aside surfaces per fence as a diagnostic block.
  ...
  // Stage 3: print through Aside — one script: serve the staged HTML over
  // loopback, (wait ≤3s for Paged.js if --toc), CDP Page.printToPDF.
  progress.begin("Rendering PDF through Aside");
  ...
  progress.end("Rendering PDF through Aside");
...
export async function preview(opts: PreviewOptions): Promise<string> {
  ...
  progress.begin("Rendering HTML");
  const markdown = fs.readFileSync(input, "utf8");
  // Preview deliberately skips the diagram/image pre-pass (no Aside
