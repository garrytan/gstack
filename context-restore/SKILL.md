...
if [ ! -d "$CHECKPOINT_DIR" ]; then
  ...
  if [ -z "$ALL" ]; then
    ...
    # Order current-worktree checkpoints first, current-branch checkpoints second, other branches after.
    # A git branch is checked out in at most one worktree, but same-name branches across clones/remotes,
    # renamed branches, or detached HEAD can lead to wrong-checkpoint restores. We partition prioritising:
    # 1. SAME_WT (Matching absolute worktree path)
    # 2. SAME_BRANCH (Matching branch name, e.g. for cross-machine restores)
    # 3. OTHER (Cross-branch fallback / Conductor handoff)
    #
    # CURRENT_BRANCH / CURRENT_WORKTREE may be pre-set (tests); otherwise resolve from git.
...
**current-worktree-first**, then **current-branch-second** (the branch and worktree are read from each file's frontmatter). Other-branch files stay in the set as a fallback, which preserves
