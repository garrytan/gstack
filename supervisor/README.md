...
| `console/styles.css` | Console design system (v7.1 — dark theme, typography tokens (`--font-body`, `--font-mono`), 8-point spacing scale (`--space-1` to `--space-8`), border-radius tokens (`--radius-card/badge/btn`), status dot colors (`--color-green/amber/red/grey`), Satoshi/DM Sans/JetBrains Mono typefaces) |
