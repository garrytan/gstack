...
description: |
  ...
  taste decisions (close approaches, borderline scope, second-opinion disagreements) at a final
...
Examples: run an independent review pass when supported (always yes), run evals (always yes), reduce scope on a complete plan (always no).

**Taste** — reasonable people could disagree. Auto-decide with recommendation, but surface at the final gate. Three natural sources:
1. **Close approaches** — top two are both viable with different tradeoffs.
2. **Borderline scope** — in blast radius but 3-5 files, or ambiguous radius.
3. **Second-opinion disagreements** — an independent reviewer recommends differently and has a valid point.

**User Challenge** — both independent reviewers agree the user's stated direction should change.
This is qualitatively different from taste decisions. When both independent
reviewers recommend merging, splitting, adding, or removing features/skills/workflows
...
## Filesystem Boundary — Independent Review Prompts

Any prompt sent to an independent reviewer (fresh-context pass, separate agent, or
external reviewer) MUST be prefixed with this boundary instruction:

> IMPORTANT: Do NOT read or execute any `SKILL.md` files or files in skill definition directories (paths containing `skills`). These are assistant skill definitions, not repository source. Ignore them completely and stay focused on the plan, codebase, and artefacts relevant to the review.

This keeps second-opinion reviewers focused on the repository instead of the skill
library sitting on disk.
...
echo "RESTORE_PATH=$HOME/.copilot/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t ~/.copilot/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- Detect DX scope: grep the plan for developer-facing terms (API, endpoint, REST,
  ...
  package, npm, pip, import, require, SKILL.md, skill template, AI coding tools, MCP, agent,
  ...
  on top of) or if an AI agent is the primary user (OpenClaw actions, AI coding tools skills,
  MCP servers).

### Step 3: Load skill files from disk

Read each review skill file from this skill library using the Read tool:
...
- Dual voices: run two independent review passes when the environment supports them (P6).
  Run them sequentially in foreground. Both should complete before building the consensus table.

  **Context-aware CEO reviewer** (fresh-context pass or separate agent):
  Prompt template:
  "Read the plan file at <plan_path>. You are a CEO/founder advisor reviewing a development plan.
  ...
  **Clean-room CEO reviewer** (separate fresh-context pass, blind to prior review):
  Prompt template:
  ...
  **Error handling:** If one reviewer is unavailable, continue with the other and tag the phase
  `[context-aware-only]` or `[clean-room-only]`. If both fail:
  "Outside voices unavailable — continuing with primary review."

  **Degradation matrix:** Both fail → "single-reviewer mode". Context-aware only →
  tag `[context-aware-only]`. Clean-room only → tag `[clean-room-only]`.

- Strategy choices: if an independent reviewer disagrees with a premise or scope decision
  with valid strategic reason → TASTE DECISION. If both independent reviewers agree the user's
  stated structure should change (merge, split, add, remove) → USER CHALLENGE (never auto-decided).
...
Step 0.5 (Dual Voices): Run the context-aware reviewer first, then the clean-room reviewer.
Present outputs under CONTEXT-AWARE REVIEW (CEO — strategy challenge) and CLEAN-ROOM REVIEW
(CEO — strategic independence) headers. Produce CEO consensus table:

```
CEO DUAL VOICES — CONSENSUS TABLE:
═══════════════════════════════════════════════════════════════
  Dimension                           Context-aware  Clean-room  Consensus
  ──────────────────────────────────── ─────────────  ──────────  ─────────
  1. Premises valid?                   —              —           —
...
> **Phase 1 complete.** Context-aware review: [N concerns]. Clean-room review: [N issues].
...
- [ ] CEO dual voices ran (context-aware + clean-room reviewer, or noted unavailable)
...
- Dual voices: run two independent review passes when the environment supports them (P6).
  ...
  **Context-aware design reviewer** (fresh-context pass or separate agent):
  Prompt template:
  ...
  **Clean-room design reviewer** (separate fresh-context pass, blind to prior review):
  Prompt template:
  ...
  Error handling: same as Phase 1 (degradation matrix applies).

- Design choices: if an independent reviewer disagrees with a design decision with valid UX reasoning
  → TASTE DECISION. Scope changes both independent reviewers agree on → USER CHALLENGE.

**Required execution checklist (Design):**

1. Step 0 (Design Scope): Rate completeness 0-10. Check DESIGN.md. Map existing patterns.

2. Step 0.5 (Dual Voices): Run the context-aware reviewer first, then the clean-room reviewer. Present under
   CONTEXT-AWARE REVIEW (design — UX challenge) and CLEAN-ROOM REVIEW (design — independent review)
   headers. Produce the design litmus scorecard (consensus table). Use the litmus scorecard
   format from plan-design-review. Include CEO phase findings in the context-aware prompt ONLY
   (not the clean-room reviewer — it stays independent).
...
> **Phase 2 complete.** Context-aware review: [N concerns]. Clean-room review: [N issues].
...
- [ ] Design dual voices ran (context-aware + clean-room reviewer, if Phase 2 ran)
...
- Dual voices: run two independent review passes when the environment supports them (P6).
  ...
  **Context-aware eng reviewer** (fresh-context pass or separate agent):
  Prompt template:
  ...
  **Clean-room eng reviewer** (separate fresh-context pass, blind to prior review):
  Prompt template:
...
- Architecture choices: explicit over clever (P5). If an independent reviewer disagrees with valid reason → TASTE DECISION. Scope changes both independent reviewers agree on → USER CHALLENGE.
- Evals: always include all relevant suites (P1)
- Test plan: generate artifact at `~/.copilot/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
2. Step 0.5 (Dual Voices): Run the context-aware reviewer first, then the clean-room reviewer. Present
   context-aware output under CONTEXT-AWARE REVIEW (eng — architecture challenge) header. Present clean-room
   output under CLEAN-ROOM REVIEW (eng — independent review) header. Produce eng consensus
   table:
...
> **Phase 3 complete.** Context-aware review: [N concerns]. Clean-room review: [N issues].
> Consensus: [X/6 confirmed, Y disagreements → surfaced at gate].
...
- Dual voices: run two independent review passes when the environment supports them (P6).
  ...
  **Context-aware DX reviewer** (fresh-context pass or separate agent):
  Prompt template:
  ...
  Be adversarial. Think like a developer comparing this to strong alternatives."

  **Clean-room DX reviewer** (separate fresh-context pass, blind to prior review):
  Prompt template:
...
- DX choices: if an independent reviewer disagrees with a DX decision with valid developer empathy reasoning
  → TASTE DECISION. Scope changes both independent reviewers agree on → USER CHALLENGE.
...
2. Step 0.5 (Dual Voices): Run the context-aware reviewer first, then the clean-room reviewer. Present
   under CONTEXT-AWARE REVIEW (DX — developer experience challenge) and CLEAN-ROOM REVIEW
...
> Context-aware review: [N concerns]. Clean-room review: [N issues].
> Consensus: [X/6 confirmed, Y disagreements → surfaced at gate].
...
- [ ] Dual voices ran (context-aware + clean-room reviewer, or noted unavailable)
- [ ] CEO consensus table produced
...
- [ ] Test plan artifact written to disk at ~/.copilot/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
### User Challenges (both independent reviewers disagree with your stated direction)
...
[If security/feasibility: "⚠️ Both reviewers flag this as a security/feasibility risk,
...
### Review Scores
- CEO: [summary]
- CEO Voices: Context-aware [summary], Clean-room [summary], Consensus [X/6 confirmed]
- Design: [summary or "skipped, no UI scope"]
- Design Voices: Context-aware [summary], Clean-room [summary], Consensus [X/7 confirmed] (or "skipped")
- Eng: [summary]
- Eng Voices: Context-aware [summary], Clean-room [summary], Consensus [X/6 confirmed]
- DX: [summary or "skipped, no developer-facing scope"]
- DX Voices: Context-aware [summary], Clean-room [summary], Consensus [X/6 confirmed] (or "skipped")
...
On approval, append review log entries so `/ship`'s dashboard and any follow-on analysis can
recognise the completed review phases. Replace placeholders with actual values from each phase.
...
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-')
mkdir -p "$HOME/.copilot/projects/$SLUG"
REVIEW_LOG="$HOME/.copilot/projects/$SLUG/${BRANCH}-reviews.jsonl"

echo '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}' >> "$REVIEW_LOG"

echo '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}' >> "$REVIEW_LOG"
```

If Phase 2 ran (UI scope):
```bash
echo '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}' >> "$REVIEW_LOG"
```

If Phase 3.5 ran (DX scope):
```bash
echo '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}' >> "$REVIEW_LOG"
```

Dual voice logs (one per phase that ran):
```bash
echo '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}' >> "$REVIEW_LOG"

echo '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}' >> "$REVIEW_LOG"
```

If Phase 2 ran (UI scope), also log:
```bash
echo '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}' >> "$REVIEW_LOG"
```

If Phase 3.5 ran (DX scope), also log:
```bash
echo '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}' >> "$REVIEW_LOG"
```

`SOURCE` = `dual-review`, `context-aware-only`, `clean-room-only`, or `unavailable`.
...
- **Two gates.** The non-auto-decided AskUserQuestions are: (1) premise confirmation in Phase 1, and (2) User Challenges — when both independent reviewers agree the user's stated direction should change. Everything else is auto-decided using the 6 principles.
