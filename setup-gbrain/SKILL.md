...
## Local completion notes (D4)

Remote reporting has been removed. If you need to summarize setup outcome, keep it in
the conversation only and use enumerated, non-secret categories such as:
...
Never expose `SUPABASE_ACCESS_TOKEN`, `DB_PASS`, `GBRAIN_POOLER_URL`,
`GBRAIN_DATABASE_URL`, or any `postgresql://` substring in output, logs, or
command arguments. The CI grep test in `test/skill-validation.test.ts` enforces
this at build time.
