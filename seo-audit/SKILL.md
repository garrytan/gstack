...
if command -v lighthouse &>/dev/null; then
  ...
  # Chrome detection — mirrors chrome-launcher's resolution order.
  # Covers Linux (google-chrome*, chromium*), macOS (.app bundle),
  # and Windows git-bash (Program Files). Honors $CHROME_PATH override.
  # We probe the binary directly rather than invoking `lighthouse about:blank`,
  # because Lighthouse validates the URL before launching Chrome — so a
  # fake-URL probe always emits INVALID_URL regardless of Chrome state.
  _chrome_ok=false
  if [ -n "$CHROME_PATH" ] && [ -x "$CHROME_PATH" ]; then
    _chrome_ok=true
  else
    for _bin in google-chrome-stable google-chrome chromium chromium-browser chrome; do
      if command -v "$_bin" >/dev/null 2>&1; then _chrome_ok=true; break; fi
    done
    [ "$_chrome_ok" = false ] && [ -x "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" ] && _chrome_ok=true
    [ "$_chrome_ok" = false ] && [ -x "/Applications/Chromium.app/Contents/MacOS/Chromium" ] && _chrome_ok=true
    [ "$_chrome_ok" = false ] && [ -x "/c/Program Files/Google/Chrome/Application/chrome.exe" ] && _chrome_ok=true
    [ "$_chrome_ok" = false ] && [ -x "/c/Program Files (x86)/Google/Chrome/Application/chrome.exe" ] && _chrome_ok=true
  fi
  if [ "$_chrome_ok" = true ]; then
    echo "LIGHTHOUSE_OK"
  else
