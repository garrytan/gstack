...
3. **Memory stage.** When the local-engine preflight permits it, stages curated memory and explicitly enabled transcripts within the enrolled scope; transcripts default to off. Use `/setup-gbrain` to choose that scope. In local-stdio MCP mode, ingests into the local engine. In remote-http MCP mode, persists scanned markdown to `~/.gstack/transcripts/run-<pid>-<ts>/` for the remote brain admin's pull pipeline; a queued write is not remote delivery. The ingest timeout is 30 minutes by default; raise it for a big brain with `GSTACK_INGEST_TIMEOUT_MS` (accepts 1 min–24h), and raise the outer `GSTACK_SYNC_MEMORY_TIMEOUT_MS` too if it would exceed that stage's default 35-minute limit. On timeout, the checkpoint is preserved. The next `/sync-gbrain` resumes only a supported, owned snapshot after rechecking its bytes, consent, and policy; unsafe or unsupported checkpoints are held for review. See [transcript enrollment and scanning](setup-gbrain/memory.md).
4. **Brain-sync stage.** Pushes curated artifacts (plans, designs, retros) and eligible queued remote-HTTP transcript pages to your private artifacts repo if configured. Transcript publication rechecks consent and scans outgoing transcript blobs; held work stays local.
