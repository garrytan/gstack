...
preamble-tier: 2
version: 0.1.0
description: Read-only dashboard for explicit release planning. (gstack)
triggers:
  - landing report
  - version queue
  - ship queue
...
WIP work likely to release soon, and what slot a release workflow would pick next. No
...
explicit release would land in. This skill is a read-only call into
`bin/gstack-next-version`, with nothing mutating.
...
1. **If there are collisions in the queue** (two open PRs claim the same version):
   ...
   to rerun the explicit release workflow to pick up the next free slot."
...
3. **If everything looks clean:**
   "Queue is clean. The next explicit release can claim a slot without conflict."
