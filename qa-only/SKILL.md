...
After the report is written, persist the final `/qa-only` outcome so `/factory-recover-qa`
can correlate this run with a pending structured factory QA dispatch.

Run:

```bash
~/.claude/skills/gstack/bin/gstack-qa-log '{"skill":"qa-only","timestamp":"TIMESTAMP","status":"STATUS","mode":"audit","summary":"SUMMARY","target_url":"TARGET_URL","target_environment":"ENV","authenticated_as":"ACCOUNT_LABEL","passed":N,"failed":N,"issues_found":N,"must_fix":N,"scenarios":SCENARIOS_JSON,"screenshots":SCREENSHOTS_JSON,"trace_steps":TRACE_STEPS_JSON}'
```

Substitute:
- `TIMESTAMP` = ISO 8601 datetime
- `STATUS` = `"clean"` if no failures, otherwise `"issues_found"`
- `mode` = always `"audit"` for `/qa-only`
- `summary` = one-line summary of the QA result
...
- `passed`/`failed`/`issues_found`/`must_fix` = counts from the report
