...
"Bash(appium:*)"
"Bash(xcrun:*)"
"Bash($GSTACK_ROOT/browse/dist/browse:*)"
"Bash($GSTACK_ROOT/browse-mobile/dist/browse-mobile:*)"
"Bash($BM:*)"
"Bash(BM=:*)"
"Bash(SID=:*)"
"Bash(JAVA_HOME=:*)"
...
# Check 2: vendored skills in project (e.g., .factory/skills/gstack/browse-mobile)
[ -z "$BM" ] && [ -n "$_ROOT" ] && [ -x "$_ROOT/.factory/skills/gstack/browse-mobile/dist/browse-mobile" ] && BM="$_ROOT/.factory/skills/gstack/browse-mobile/dist/browse-mobile"
# Check 3: global gstack install (works from ANY project directory)
# browseDir is e.g. $GSTACK_ROOT/browse/dist — go up 2 levels to gstack root
[ -z "$BM" ] && [ -x $GSTACK_BROWSE/../../browse-mobile/dist/browse-mobile ] && BM=$GSTACK_BROWSE/../../browse-mobile/dist/browse-mobile
if [ -n "$BM" ] && [ -x "$BM" ]; then
...
3b. **Mobile project detection** — if `$BM` is available (MOBILE_READY from setup):
   ...
   **Findings:**
   ...
   - Take screenshots at milestones and read the file to show them to the user
