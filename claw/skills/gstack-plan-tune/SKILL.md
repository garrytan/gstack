...
7. **"Turn it off" / "disable"** → `~/.claw/skills/gstack/bin/gstack-config set question_tuning false`
8. **"Turn it on" / "enable"** → `~/.claw/skills/gstack/bin/gstack-config set question_tuning true`
...
1. Read the current state:
   ```bash
   _QT=$(~/.claw/skills/gstack/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
...
3. If A or B: enable:
   ```bash
   ~/.claw/skills/gstack/bin/gstack-config set question_tuning true
   ```
...
4. If A (full setup), ask FIVE one-per-dimension declaration questions via
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   ~/.claw/skills/gstack/bin/gstack-developer-profile --read >/dev/null
   # Update declared dimensions atomically
   eval "$(~/.claw/skills/gstack/bin/gstack-paths)"
...
```bash
~/.claw/skills/gstack/bin/gstack-developer-profile --profile
```
...
4. Write:
   ```bash
   ~/.claw/skills/gstack/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.claw/skills/gstack/bin/gstack-developer-profile --gap
```
...
```bash
~/.claw/skills/gstack/bin/gstack-question-preference --stats
eval "$(~/.claw/skills/gstack/bin/gstack-slug 2>/dev/null)"
eval "$(~/.claw/skills/gstack/bin/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
[ -f "$_LOG" ] && echo "TOTAL_LOGGED: $(wc -l < "$_LOG" | tr -d ' ')" || echo "TOTAL_LOGGED: 0"
~/.claw/skills/gstack/bin/gstack-developer-profile --profile | bun -e "
