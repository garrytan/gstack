...
~/.claw/skills/gstack/bin/gstack-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
```bash
eval "$(~/.claw/skills/gstack/bin/gstack-slug 2>/dev/null)"
~/.claw/skills/gstack/bin/gstack-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(~/.claw/skills/gstack/bin/gstack-slug 2>/dev/null)"
~/.claw/skills/gstack/bin/gstack-learnings-search --limit 100 2>/dev/null
```
...
Export learnings as markdown suitable for adding to CLAW.md or project documentation.

```bash
eval "$(~/.claw/skills/gstack/bin/gstack-slug 2>/dev/null)"
~/.claw/skills/gstack/bin/gstack-learnings-search --limit 50 2>/dev/null
```
...
Present the formatted output to the user. Ask if they want to append it to CLAW.md
...
```bash
~/.claw/skills/gstack/bin/gstack-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```
