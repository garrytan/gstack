...
```bash
~/.claw/skills/gstack/bin/gstack-gbrain-detect
```
...
```bash
~/.claw/skills/gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.claw/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.claw/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.claw/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.claw/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.claw/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.claw/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.claw/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
- `SERVER_VERSION` (e.g., `0.27.1`) — written to the CLAW.md block in Step 8.
...
(per-remote policy) → Step 7 (artifacts repo) → Step 8 (CLAW.md) → Step 9
...
```bash
current_tier=$(~/.claw/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.claw/skills/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.claw/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.claw/skills/gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.claw/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
...
After memory sync is wired (Step 7) but before persisting the CLAW.md
...
```bash
~/.claw/skills/gstack/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
~/.claw/skills/gstack/bin/gstack-config set transcript_ingest_mode <choice>
~/.claw/skills/gstack/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
Reference doc for users: `setup-gbrain/memory.md` (linked from CLAW.md
Step 8).

---

## Step 8: Persist `## GBrain Configuration` in CLAW.md
...
- Token: stored in ~/.claude.json (do not commit; never written to CLAW.md)
...
The bearer token is **never** written to CLAW.md (CLAW.md is checked
...
```bash
~/.claw/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.claw/skills/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.claw/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
...
- **CLAW.md is the audit trail.** Always update it in Step 8 after a
  successful setup.
