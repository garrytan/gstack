...
if [ ! -f ~/.gstack/projects/$SLUG/land-deploy-confirmed ]; then
  ...
  CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAW.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
...
```bash
# Check for persisted deploy config in CLAW.md
DEPLOY_CONFIG=$(grep -A 20 "## Deploy Configuration" CLAW.md 2>/dev/null || echo "NO_CONFIG")
echo "$DEPLOY_CONFIG"
...
If `PERSISTED_PLATFORM` and `PERSISTED_URL` were found in CLAW.md, use them directly
...
and any persisted config from CLAW.md.
...
1. **CLAW.md persisted config:** Check for a staging URL in the Deploy Configuration section:
```bash
grep -i "staging" CLAW.md 2>/dev/null | head -3
```
...
```bash
~/.claw/skills/gstack/bin/gstack-review-read 2>/dev/null
```
...
**If C:** **STOP.** "Running `/setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to CLAW.md so I'll know exactly what to do next time. Run `/land-and-deploy` again when that's done."
...
```bash
cat ~/.claw/skills/gstack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
Read CLAW.md to find the project's test command. If not specified, use `bun test`.
...
```bash
git diff --name-only $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main)...HEAD -- README.md CHANGELOG.md ARCHITECTURE.md CONTRIBUTING.md CLAW.md VERSION
```
...
```bash
eval $(~/.claw/skills/gstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
If staging was detected in Step 1.5c (or from CLAW.md deploy config), and the changes
...
If a deploy status command was configured in CLAW.md (e.g., `fly status --app myapp`), use it instead of or in addition to GitHub Actions polling.
...
If CLAW.md has a custom deploy status command in the "Custom deploy hooks" section, run that command and check its exit code.
