...
description: |
  ...
  guidance in CLAW.md. Wraps the gstack-gbrain-sync orchestrator with
...
guidance in CLAW.md so the coding agent knows when to prefer `gbrain`
search over Grep.
...
```bash
~/.claw/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null
```
...
(we'd write a CLAW.md guidance block referencing tools that don't exist).
...
```bash
bun run ~/.claw/skills/gstack/bin/gstack-gbrain-sync.ts <user-args>
```
...
## Step 4: Refresh `## GBrain Search Guidance` block in CLAW.md
...
Then update CLAW.md based on capability state:
...
end of CLAW.md if it doesn't. NEVER duplicate. Block is machine-AGNOSTIC
...
the entire block at the end of CLAW.md.

**Atomic write:** write the new CLAW.md content to a tmp file alongside it
(e.g., `CLAW.md.sync-gbrain.tmp`) then `mv` to atomic-rename, so a crash
...
Do NOT crash if CLAW.md is missing or unwritable — log a warning and
continue.
...
gbrain status: GREEN
  ...
  CLAW.md ....... OK   ## GBrain Search Guidance present
...
check failed; CLAW.md guidance block REMOVED — run /setup-gbrain to repair`).
...
any state-file or CLAW.md mutation and exits with code 2 if another sync is
in flight. Stale locks (process died) auto-clear after 5 minutes.

## Cross-machine note

The `## GBrain Search Guidance` block is committed to the repo's CLAW.md
...
CLAW.md but no local gbrain, /sync-gbrain detects the mismatch via the
...
- **DONE** — all stages green, CLAW.md guidance block present, verdict GREEN.
