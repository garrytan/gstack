...
[ -z "$_AUTO" ] && _AUTO=$($_GSTACK_DIR/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
$_GSTACK_DIR/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
$_GSTACK_DIR/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `$_GSTACK_DIR/bin/gstack-config set update_check true` to re-enable."
...
INSTALL_TYPE=""
INSTALL_DIR=""
# Check all possible agent skill directories
for _prefix in "$HOME/.claude/skills" "$HOME/.agents/skills" ".claude/skills" ".agents/skills"; do
  if [ -d "$_prefix/gstack/.git" ]; then
    if [ "$_prefix" = "$HOME/.claude/skills" ] || [ "$_prefix" = "$HOME/.agents/skills" ]; then
      INSTALL_TYPE="global-git"
    else
      INSTALL_TYPE="local-git"
    fi
    INSTALL_DIR="$_prefix/gstack"
    break
  elif [ -d "$_prefix/gstack" ]; then
    if [ "$_prefix" = "$HOME/.claude/skills" ] || [ "$_prefix" = "$HOME/.agents/skills" ]; then
...
LOCAL_GSTACK=""
for _ldir in "$_ROOT/.claude/skills/gstack" "$_ROOT/.agents/skills/gstack"; do
  if [ -n "$_ROOT" ] && [ -d "$_ldir" ]; then
    _RESOLVED_LOCAL=$(cd "$_ldir" && pwd -P)
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit it when you're ready."
...
```bash
$_GSTACK_DIR/bin/gstack-update-check --force 2>/dev/null || true
```
