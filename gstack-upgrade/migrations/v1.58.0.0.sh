...
#
# Why a migration: v1.58 made the PreToolUse question-preference-hook carry
# Conductor-specific routing. But setup's hook-install block skips silently in non-interactive
...
# Only relevant inside Conductor — this historical migration is host-scoped.
...
case "${_PT}" in
  n|no|false|skip|off|0)
    echo "  [v1.58.0.0] plan_tune_hooks opted out — leaving Conductor hooks unchanged." >&2
    touch "${DONE}"
    exit 0
    ;;
esac
...
if [ -x "${SETTINGS_HOOK}" ] && [ -x "${PREF_HOOK}" ]; then
  "${SETTINGS_HOOK}" add-event \
    ...
    && echo "  [v1.58.0.0] Conductor AskUserQuestion hook registered (PreToolUse)." >&2 \
