...
[ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "orchestrator-advisor-step3-$(date -u +%Y%m%d)" "pipeline-orchestrator" "claude-opus-4-7" "Step 3 advisor: 참여 부서장 결정"
```
...
> ```
>
> **일반화 가능성 가드 (필수, retro-protocol.md §3-1 준수)**:
