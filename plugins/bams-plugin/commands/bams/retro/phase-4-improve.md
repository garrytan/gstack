...
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "hr-agent-9-$(date -u +%Y%m%d)" "hr-agent" "claude-opus-4-7[1m]" "Step 9: 에이전트 파일 수정"
```

서브에이전트 실행 (Task tool, subagent_type: **"bams-plugin:hr-agent"**, model: **"claude-opus-4-7[1m]"**):
