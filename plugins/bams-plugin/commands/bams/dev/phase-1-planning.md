...
Bash로 agent_start emit:
```bash
_EMIT=$(find ~/.claude/plugins/cache -name "bams-viz-emit.sh" -path "*/bams-plugin/*" 2>/dev/null | head -1); [ -n "$_EMIT" ] && bash "$_EMIT" agent_start "{slug}" "cross-department-coordinator-4-$(date -u +%Y%m%d)" "cross-department-coordinator" "opus" "Step 4: 기획→구현 핸드오프 조율"
```
