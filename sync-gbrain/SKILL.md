...
shared read-only, with persistence to `brain_trust_policy@<hash>` and a
conditional `artifacts_sync_mode=full` flip for personal or
`artifacts_sync_mode=artifacts-only` for shared contributors). Then continue.
