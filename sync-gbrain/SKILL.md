...
preamble-tier: 2
version: 1.0.0
description: Sync this repository's changed Markdown and code into gbrain. (gstack)
triggers:
  - sync gbrain
  - refresh gbrain
  - reindex repo
...
Refresh agent search guidance in CLAUDE.md through the gstack-gbrain-sync
orchestrator with state probing, native repository-source registration, capability checks,
...
**Architecture (post-codex review):** This skill uses gbrain's native source
surfaces (`gbrain sources add`, `gbrain sync --strategy auto`,
`gbrain reindex-code`, `gbrain code-def/code-refs/code-callers/code-callees`).
The automatic walk admits eligible changed Markdown and code. Repository
images are deliberately disabled on this path until GBrain preserves the
selected source for image imports. It does NOT route the entire repository
through `gbrain import`.
...
- `/sync-gbrain` — incremental repository sync (default; changed Markdown/code; mtime fast-path; ~50ms steady-state)
- `/sync-gbrain --full` — run the same automatic repository walk, then a full code reindex via `gbrain reindex-code` (~25-35 min on a big repo). A healthy existing anchor stays incremental; GBrain may perform a full reconciliation on first sync, when its anchor/history is unavailable, or when its indexing pipeline changes. Auto-builds the call graph (`gbrain dream`) **only when it was never built**.
...
- `/sync-gbrain --code-only` — only run the repository stage; skip memory + brain-sync (legacy flag name retained for compatibility)
...
**Split-engine model (v1.34.0.0+).** Repository stage runs locally against the
...
Practically: local PGLite holds the worktree's repository source (changed
Markdown and code) on remote-http machines; the remote
brain holds everything else. Local-stdio machines mix repository contents +
...
- **`missing-config`** AND `gbrain_mcp_mode == "remote-http"`: tell the user
  ...
  repository and symbol search needs a local PGLite. Run `/setup-gbrain` and
  pick 'Yes' at the local repository index prompt (Step 4.5), or run
...
The orchestrator runs three stages: repository → memory → brain-sync (per the
...
## Step 3: Repository-index health check
...
> D1 — This repo has 0 indexed pages in gbrain. Run repository sync + full code reindex now?
>
> ELI10: gbrain hasn't indexed this repository yet. The semantic search
> tools (`gbrain search`, `code-def`, `code-refs`) will return nothing
> until it has pages. The auto pass admits eligible new/changed Markdown and
> code; the second pass reindexes code. A healthy existing anchor stays
> incremental, but GBrain can fully reconcile the repository when the anchor
> or history is unavailable or its indexing pipeline changes. Takes ~25-35
> minutes on a big Mac.
>
...
semantic results match the repository files on disk here.
...
- This worktree's Markdown and code, auto-pinned via `.gbrain-source`.
  Repository images stay excluded until GBrain preserves source isolation for
  image imports.
...
Scheduled repository-wide Markdown + code refresh is not configured by this
skill; GBrain autopilot currently uses its source's own strategy.

Safety: don't run `/sync-gbrain` while `gbrain autopilot` is active — the
orchestrator refuses destructive source ops when it detects a running autopilot
to avoid racing it (#1734). Prefer registering user repos with `gbrain sources
add --path <dir>` (no `--url`): URL-managed sources can auto-reclone, and the
repository sync for them requires an explicit `--allow-reclone` opt-in.
