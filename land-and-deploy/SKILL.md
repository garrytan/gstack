...
Capture the PR head and its current checks:

```bash
PR_HEAD_SHA=$(gh pr view --json headRefOid -q .headRefOid)
gh pr checks --json name,state,workflow
```

If all required checks belong to `PR_HEAD_SHA`, all are successful, and at least
one check is clearly a test/build/typecheck job, accept that as exact-SHA test
evidence. Do not run the same suite locally again. Record the check names and SHA
