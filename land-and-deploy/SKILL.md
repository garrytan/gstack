...
- **Pre-merge readiness warnings or blockers (Step 3.5)** — a fully green report proceeds automatically
...
3. If all checks pass (or no required checks): Tell the user "CI passed." Skip Step 3, go to Step 3.4.
...
If CI passes within the timeout: Tell the user "CI passed after {duration}. Moving to readiness checks." Continue to Step 3.4.
...
**Free tests — prefer exact-SHA CI evidence:**

Bracket the check query with the PR head so a push racing the readiness check
cannot make stale CI look current:

```bash
PR_HEAD_SHA_BEFORE=$(gh pr view --json headRefOid -q .headRefOid)
CI_CHECKS_JSON=$(gh pr checks --json name,state,workflow)
PR_HEAD_SHA_AFTER=$(gh pr view --json headRefOid -q .headRefOid)
```

Accept the CI result only when `PR_HEAD_SHA_BEFORE == PR_HEAD_SHA_AFTER`, every
required check in `CI_CHECKS_JSON` is successful, and at least one check is clearly
a test/build/typecheck job. This stable-head bracket proves the checks came from
the exact PR head being assessed. Do not run the same suite locally again. Record
the check names and `PR_HEAD_SHA_AFTER` in the readiness report.

Run local free tests only when CI is absent, has no test/build check, targets a
different SHA, or the project explicitly documents an additional local-only gate.
This fallback discovers the command from project instructions; it does not assume
`bun test` for non-Bun repositories.
...
If local fallback tests fail: **BLOCKER.** Cannot merge with failing tests.
...
If there are zero warnings and zero blockers, print the report and continue to
Step 4 automatically. The explicit `/land-and-deploy` invocation is authorization
to merge; a second green confirmation adds latency without information.

If there are warnings or blockers, use AskUserQuestion:
