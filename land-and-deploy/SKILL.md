...
- Missing required validation commands or unknown/unmet requested human review (Step 3.5)
...
2. Parse arguments. If the user specified `#NNN`, use that PR number. If a URL was provided, save it for canary verification in Step 7. Set `PR_NUMBER` to the resolved PR and use it for every subsequent PR query and merge, even when it differs from the checked-out branch. Resolve its base repository as `{owner}/{repo}`; do not query a fork's review history by mistake.
...
Use the supported `bucket` classification with `state` for detail. Inspect output
even on nonzero exit (`gh pr checks` uses exit 8 for pending); an API/auth/parse
failure is UNKNOWN, never green. `fail` or `cancel` means **STOP**; `pending` means
wait in Step 3. `skipping` is not a successful run: confirm the project's declared
CI/branch rules allow that omission, otherwise stop. Unknown buckets/states block.
Only all applicable checks at `pass` means "CI passed." An empty check list needs
confirmation from the declared CI and applicable branch rules that no checks are
required; report "No CI checks required", not "CI passed". Missing required checks
block. If clear, skip only Step 3's wait loop; continue to Step 3.4, then Step 3.5.
...
If the watch completes: rerun Step 2's JSON check and classification; a completed
watch alone does not prove CI passed. When clear, report the outcome and duration,
then continue to Step 3.4, then Step 3.5 before merging.
