...
3. **Vercel/Netlify preview deploys:** Read the exact PR's supported check snapshot:
```bash
$GSTACK_ANCHOR_INVOCATION gstack-pr-checks snapshot --pr "$PR_NUMBER"
```
The adapter derives the canonical repository selector, verifies repository identity and
read permission, and requests only `name,state,bucket,link,workflow`. Look for check names
containing "vercel", "netlify", or "preview" and treat their supported `link` as informational.
...
**If A:** Tell the user: "Great — the current deploy configuration is confirmed
for this run." Do not save a machine marker or configuration fingerprint.
Continue to Step 2.
