...
**Web search:** Codex can look up docs and APIs during review. Web search is enabled
by default in current Codex, so no flag is needed (the old `--enable web_search_cached`
is deprecated).
