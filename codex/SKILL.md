...
**Note:** `codex exec resume` accepts a much smaller flag set than `codex exec`.
It does NOT accept `-s/--sandbox` or `-C/--cd`. Passing either fails with
`error: unexpected argument`. So:
- Force read-only with `-c 'sandbox_mode="read-only"'`. Without it the resumed
  session inherits whatever sandbox policy the Codex CLI's own config file sets,
  which can mean write access on round 2+.
- Set the working directory by `cd`-ing first, not with `-C`.
- Keep the `< /dev/null` redirect so a non-interactive run cannot hang on stdin.

```bash
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT" && codex exec resume <session-id> "<prompt>" -c 'sandbox_mode="read-only"' -c 'model_reasoning_effort="medium"' --enable web_search_cached --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 python3 -u -c "
