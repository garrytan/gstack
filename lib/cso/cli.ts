...
async function start(args:string[],dependencies:CsoCliDependencies,parent?:RunReportV3['parent'],requiredAncestor?:string):Promise<RunReportV3>{
  const {repo,policy,comparisonBase}=parseStart(args);assertStateOutside(repo);if(!parent)retention();const createdAt=new Date(),run=newRun(repo);
...
async function recheck(args:string[],dependencies:CsoCliDependencies){if(!args.length)throw new CsoError('INVALID_ARGUMENT','recheck requires a finding ID');retention();const findingId=args.shift()!,runId=need(args,'--run'),originalDir=runDirectory(runId);return await withLock(originalDir,async()=>{const original=loadReport(originalDir),finding=original.findings.find(f=>f.id===findingId);if(!finding)throw new CsoError('MISSING_INPUT','Original finding does not exist');if(original.status!=='finished')throw new CsoError('INVALID_SCHEMA','Recheck requires a finished original audit');
...
async function withReplayBundle<T>(id:string,fn:(stored:{path:string;dir:string},bundle:any)=>Promise<T>):Promise<T>{if(!/^[a-f0-9]{32}$/.test(id))throw new CsoError('INVALID_ARGUMENT','Bundle identifier must be the 32-character ID returned by verify');const root=privateRoot();for(const repo of fs.readdirSync(root).filter(x=>/^[a-f0-9]{24}$/.test(x)))for(const run of fs.readdirSync(join(root,repo)).filter(x=>/^\d{13}-[a-f0-9]{16}$/.test(x))){const stored={dir:join(root,repo,run),path:join(root,repo,run,'bundles',`${id}.json`)};if(!fs.existsSync(stored.path))continue;let matched=false,value!:T;await withLock(stored.dir,async()=>{if(!fs.existsSync(stored.path))return;const bundle=replayBundle(stored,id);matched=true;value=await fn(stored,bundle);});if(matched)return value;}throw new CsoError('MISSING_INPUT','Repair bundle was not found or expired');}
...
async function replay(args:string[],dependencies:CsoCliDependencies){if(!args.length)throw new CsoError('INVALID_ARGUMENT','replay requires a bundle ID');const id=args.shift()!,source=args.includes('--source')?callerPath(need(args,'--source')):undefined;if(args.length)throw new CsoError('INVALID_ARGUMENT',`Unknown replay argument: ${args[0]}`);return await withReplayBundle(id,async(stored,bundle)=>{
  ...
  let workDir=stored.dir,manifest:any,temporary:string|undefined;const retained=join(stored.dir,'snapshot'),replayDeadline=Date.now()+300_000;
  try{
    if(fs.existsSync(retained)){manifest=readJson(join(stored.dir,'snapshot.json'));assertSnapshot(stored.dir,manifest);}else if(source){const temp=newRun(source);temporary=temp.dir;workDir=temp.dir;manifest=await capture(source,workDir,undefined,undefined,{deadlineMs:replayDeadline});if(manifest.executionHash!==bundle.requiredInputs.sourceHash||manifest.originalHash!==bundle.requiredInputs.originalHash)throw new CsoError('INCOMPATIBLE_INPUT','Supplied source does not match the bundle input hashes');}else throw new CsoError('MISSING_INPUT','Retained source expired; supply explicitly matching source');
