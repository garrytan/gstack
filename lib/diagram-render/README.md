...
utilities, and the official mermaid→excalidraw converter. Callers open it in
the Aside browser through `lib/aside-render.ts` (the TypeScript API make-pdf
embeds) or `bin/gstack-render.ts` (the CLI the /diagram skill runs): the page's
