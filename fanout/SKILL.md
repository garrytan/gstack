...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"fanout","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
You take a finished design doc on disk and turn it into a parallel execution plan: N independent slabs of work, a synchronous Slab 0 for shared groundwork, and a dispatch script that can spawn worktrees plus agents. You produce two artifacts: a `## Parallel Execution Plan` section appended to the input doc, and a `worktree-dispatch.sh` sidecar next to it.
...
4. **Natural seams.** Backend/frontend, schema/logic/UI, API/client/CLI, server/extension. Apply by reading section headings and content.
...
### Step 9: Write worktree-dispatch.sh

Write to the same directory as the input doc. If input is `docs/designs/FOO.md`, write `docs/designs/worktree-dispatch.sh`.

Compute helpers:
- `<repo-name>` = `basename $(git rev-parse --show-toplevel)`
- `<topic>` = lowercase, hyphenated stem of the input filename (e.g., `FANOUT.md` → `fanout`).

**Substitution discipline** (same as Step 8): every `<...>` token below is a placeholder. Substitute concrete values from the matrix before writing. Do NOT leave `<file list>`, `<gate>`, or the meta-string `"...Slab N prompt..."` literal in the output.

The script has two parts:

1. **Slab 0 block** (uncommented, at the top): `git worktree add` + a `claude -p` heredoc whose prompt names Slab 0's Writes and Verification gate.
2. **One commented block per non-Slab-0 slab**: same shape as Slab 0, but commented with `#` so the user uncomments it manually after Slab 0 lands. Each block's heredoc must be populated with THAT slab's Writes and gate.

If Slab 0 is non-empty, write a script in this shape:
...
# # Each block must have its own populated <file-list> and <gate> from the matrix.
#
# wait
```
...
- Files written: `<doc-path>` (appended), `<dispatch-script-path>` (created, +x).
- Next step: review the appended section in the doc, then run `bash <dispatch-script-path>` when ready.
...
2. **Slab 0 is empty.** Section still written with "no shared groundwork detected"; dispatch script omits Slab 0 block; Slabs 1-N uncommented from the start.
...
2. NEVER write to files outside the input doc's directory. Two files only: the doc itself (appended) and the dispatch script (created alongside).
