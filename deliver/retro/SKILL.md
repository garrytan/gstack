...
Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Local-first, tool-agnostic, and compatible with both repo-scoped retros and optional cross-project discovery when a helper script is configured.
...
First, refresh remotes, detect the default branch, and identify the current user:
```bash
git fetch origin --quiet 2>/dev/null || true
DEFAULT=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's@^refs/remotes/origin/@@')
[ -z "$DEFAULT" ] && for b in main master; do git show-ref --verify --quiet "refs/remotes/origin/$b" && DEFAULT="$b" && break; done
...
# 8. Review signal history (if available)
cat ~/.copilot/review-history.md 2>/dev/null || cat ~/.copilot/greptile-history.md 2>/dev/null || true
...
| Review signal | N% (Y valid catches, Z false positives) |
...
**Review signal (if history exists):** Read `~/.copilot/review-history.md` if present, otherwise fall back to `~/.copilot/greptile-history.md` for backward compatibility (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or neither file exists, skip the Review signal metric row. Skip unparseable lines silently.
...
**Eureka Moments (if logged):** Read `~/.copilot/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```
  EUREKA /office-hours (branch: alex/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA /plan-eng-review (branch: alex/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
- Review signal ratio and trend (if history exists): "Review signal: X% (Y valid catches, Z false positives)"
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
cat ~/.copilot/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
Locate and run a configured discovery script using this fallback chain:

```bash
DISCOVER_BIN=""
[ -n "$COPILOT_RETRO_DISCOVER" ] && DISCOVER_BIN="$COPILOT_RETRO_DISCOVER"
[ -z "$DISCOVER_BIN" ] && [ -x "$HOME/.copilot/analytics/retro-global-discover" ] && DISCOVER_BIN="$HOME/.copilot/analytics/retro-global-discover"
[ -z "$DISCOVER_BIN" ] && [ -x "./bin/retro-global-discover" ] && DISCOVER_BIN="./bin/retro-global-discover"
[ -z "$DISCOVER_BIN" ] && command -v retro-global-discover >/dev/null 2>&1 && DISCOVER_BIN="$(command -v retro-global-discover)"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Configure `COPILOT_RETRO_DISCOVER` or install a compatible `retro-global-discover` helper to use `/retro global`." and stop.

Run the discovery:
```bash
"$DISCOVER_BIN" --since "<window>" --format json 2>/tmp/discover-stderr
```

Read the stderr output from `/tmp/discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Behavioral patterns (e.g., one tool used exclusively for repo A, another shared across everything else)
...
**Tweetable summary** (first line, before everything else):
```
Week of Mar 14: 5 projects, 138 commits, 250k LOC across 5 repos | 48 AI sessions | Top tools: Copilot 31, Gemini 9, Other 8 | Streak: 52d 🔥
```
...
║  [N] AI coding sessions ([tool breakdown])
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not a list of low-level helper commits.
...
| Total LOC | +N / -N |
| AI coding sessions | N total ([tool breakdown]) |
| Active days | N |
...
- List each tool present in the discovery JSON with session count, repo coverage, and notable behavioural patterns
- Highlight whether any tool is dominant, niche, or reserved for a particular kind of work
- Call out tool overlap only if it reveals something useful about workflow or context switching
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.copilot/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.copilot/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.copilot/retros/global-${today}-${next}.json`:
  ...
  "projects": [
    {
      ...
      "deletions": 800,
        "sessions_total": 18,
        "sessions_by_tool": { "copilot": 15, "other": 3 }
    }
  ],
  "totals": {
    "commits": 182,
    ...
    "active_days": 6,
    "sessions_total": 59,
    "sessions_by_tool": { "copilot": 48, "other": 11 },
    "global_streak_days": 52,
    "avg_context_switches_per_day": 2.1
  },
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | 59 AI sessions | Top tool: Copilot (48) | Focus: repo-name (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.copilot/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
