...
name: ship
description: |
  Ship workflow: detect and merge base branch, run tests, review diff, bump VERSION,
  update CHANGELOG, commit, push, create PR. Use when asked to "ship", "deploy",
  "push to main", "create a PR", "merge and push", or "get it deployed".
  Proactively suggest when the user says code is ready, asks about deploying,
...
For Design Review: inspect `git diff --name-only <base>...HEAD` for frontend files, templates, styles, or user-facing flows. If frontend scope is present and no design review has been run, mention: "Design Review not run — this PR changes frontend code. Step 3.5 will still run code review, but consider `/plan-design-review` before implementation work or `/design-review` for a fuller UX audit after implementation." Still never block.
...
1. Read `review/checklist.md` from this skills library. If the file cannot be read, **STOP** and report the error.
...
```bash
REMOTE_SLUG=$(git remote get-url origin 2>/dev/null | sed -E 's#(git@github.com:|https://github.com/)##; s#\.git$##')
SLUG="${REMOTE_SLUG:-$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")}"
BRANCH=$(git branch --show-current)
mkdir -p "$HOME/.copilot/projects/$SLUG"
echo '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}' >> "$HOME/.copilot/projects/$SLUG/$BRANCH-reviews.jsonl"
```
...
Read `review/greptile-triage.md` from this skills library and follow the fetch, filter, classify, and **escalation detection** steps.
...
Read `review/TODOS-format.md` from this skills library for the canonical format reference.

**1. Check if TODOS.md exists** in the repository root.

**If TODOS.md does not exist:** Use AskUserQuestion:
- Message: "This skills library recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
- Options: A) Create it now, B) Skip for now
- If A: Create `TODOS.md` with a basic skeleton (`# TODOS`, one workstream heading, priority buckets, and a Completed section). Continue to step 3.
...
- Priority buckets organized as `### P0` through `### P4`
...
🤖 Generated with [AI coding tools](https://claude.com/claude-code)
```
...
Append to `~/.copilot/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.copilot/projects/$SLUG/$BRANCH-reviews.jsonl
```
...
- **Never ask for trivial confirmations** (e.g., "ready to push?", "create PR?"). DO stop for: version bumps (MINOR/MAJOR), pre-landing review findings (ASK items), and secondary-model structured review [P1] findings (large diffs only).
