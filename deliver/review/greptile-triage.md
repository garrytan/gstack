...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || git rev-parse --show-toplevel 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.copilot/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || git rev-parse --show-toplevel 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.copilot/projects/$REMOTE_SLUG"
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.copilot/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.copilot/greptile-history.md` (global aggregate)
